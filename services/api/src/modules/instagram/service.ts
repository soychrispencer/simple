...
import { InstagramAnalyticsService, InstagramAnalytics } from './analytics.js';
import { InstagramABTestingService, ABTestCampaign } from './ab-testing.js';
import { InstagramSchedulerService, ScheduledPost } from './scheduler.js';
import { analyzeListingForTemplate, generateSmartTemplates as generateSmartTemplatesFromLib, ListingData } from './templates.js';

const igLogger = logger.context('instagram');
...
export async function exchangeToLongLivedToken(shortLivedToken: string): Promise<{
    ...
    try {
        ...
        igLogger.error('long-lived exchange error', error);
        return null;
    }
}
...
export async function publishInstagramCarousel(input: {
    ...
    for (const [index, img] of input.images.slice(0, 10).entries()) {
        ...
        if (!imageUrl) {
            igLogger.error(`error en item ${index + 1}: URL de imagen vacía`);
            continue;
        }
        ...
        igLogger.info(`creando item carrusel ${index + 1}/${input.images.length}`, { imageUrl });
        ...
        while (!itemCreated && itemAttempts < maxItemAttempts) {
            try {
                ...
                if (res.id) {
                    ...
                    itemCreated = true;
                    igLogger.info(`item carrusel ${index + 1} creado exitosamente`, { attempt: itemAttempts + 1 });
                }
            } catch (error) {
                itemAttempts++;
                ...
                if (itemAttempts < maxItemAttempts && (
                    ...
                    igLogger.info(`reintentando item carrusel ${index + 1} en 5s`, { attempt: itemAttempts, maxAttempts: maxItemAttempts });
                    await new Promise(resolve => setTimeout(resolve, 5000));
                    continue;
                }
                
                igLogger.error(`error creando item carrusel ${index + 1} tras ${itemAttempts} intentos`, error);
                ...
                igLogger.warn(`continuando sin item ${index + 1} del carrusel`);
                break;
            }
        }
    }
    ...
    igLogger.info('publicando carrusel', { creationId });
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            if ((msg.includes('not ready') || msg.includes('not available') || msg.includes('9007')) && attempts < maxAttempts - 1) {
                attempts++;
                igLogger.info(`carrusel no listo (intento ${attempts}/${maxAttempts}). esperando 10s...`);
...
export async function publishInstagramImage(input: {
    ...
    igLogger.info('publicando contenedor (con reintentos si es necesario)', { creationId });
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            if (isNotReady && attempts < maxAttempts - 1) {
                attempts++;
                igLogger.info(`el medio no esta listo (intento ${attempts}/${maxAttempts}). esperando 5s...`);
                await new Promise(resolve => setTimeout(resolve, 5000));
                continue;
            }
            throw error;
...
async function requestInstagram<T>(url: string, init: RequestInit, retryCount = 0): Promise<T> {
    ...
    if (!response.ok) {
        ...
        igLogger.error('Meta API ERROR', new Error(`Status ${response.status}: ${JSON.stringify(errorObject)}`));
        ...
        if (isTransient && retryCount < 3) {
            ...
            igLogger.info(`error transitorio detectado, reintentando en ${delay}ms`, { attempt: retryCount + 1, maxAttempts: 3 });
...
export async function publishListingToInstagramEnhanced(
    ...
    try {
        ...
        igLogger.error('Error en publicación mejorada', error);
        return {
            success: false,
