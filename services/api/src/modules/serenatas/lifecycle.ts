...
import { normalizeCancellationHours, validateCancellationLead } from '@simple/utils';
...
function eventWallClockToMs(ymd: string, minutes: number, timezone = SERENATA_TIMEZONE): number | null {
...
export async function cancelClientPendingSerenata(
    ...
    if (pending.eventTime && pending.providerGroupId && pending.paymentStatus === 'paid') {
        const providerGroup = await db.query.serenataProviderGroups.findFirst({
            where: eq(serenataProviderGroups.id, pending.providerGroupId),
        });
        const cancellationHours = normalizeCancellationHours(providerGroup?.cancellationHours);
        const ymd = eventDateYmd(pending.eventDate);
        if (ymd) {
            const eventMinutes = minutesFromEventTime(pending.eventTime);
            if (eventMinutes != null) {
                const eventMs = eventWallClockToMs(
                    ymd,
                    eventMinutes,
                    providerGroup?.timezone ?? SERENATA_TIMEZONE,
                );
                if (eventMs != null) {
                    const hoursUntil = (eventMs - Date.now()) / (1000 * 60 * 60);
                    const leadError = validateCancellationLead(hoursUntil, cancellationHours);
                    if (leadError) {
