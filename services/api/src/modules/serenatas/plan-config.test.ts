...
describe('serenataProMonthlyChargeClp', () => {
    it('incluye IVA 19% sobre el neto mensual', () => {
        ...
        expect(serenataPlanMonthlyChargeClp('essential')).toBe(Math.round(SERENATA_ESSENTIAL_PRICE_MONTHLY_CLP * 1.19));
        expect(serenataPlanMonthlyChargeClp('pro')).toBe(Math.round(SERENATA_PRO_PRICE_MONTHLY_CLP * 1.19));
    });
});
