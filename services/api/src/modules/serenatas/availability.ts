...
import { expandWeeklyRulesToDaySegments, BUSINESS_SCHEDULE_ALWAYS_OPEN_RULES, BUSINESS_SCHEDULE_TYPICAL_WEEK_SAVE, normalizeBookingWindowDays, normalizeCancellationHours, validateBookingWindowDate } from '@simple/utils';
...
export type ProviderGroupSlotOptions = {
    dayStart?: string;
    dayEnd?: string;
    segments?: Array<{ startTime: string; endTime: string; breakStart?: string | null; breakEnd?: string | null }>;
...
export const DEFAULT_WEEKLY_RULES: SerenataAvailabilityRuleInput[] = BUSINESS_SCHEDULE_TYPICAL_WEEK_SAVE.map((rule) => ({
...
export async function getProviderGroupSlotOptions(
    ...
    const group = await db.query.serenataProviderGroups.findFirst({
        ...
        columns: { bufferMinutes: true, timezone: true, alwaysOpen: true },
    });
    ...
    const sourceRules = group.alwaysOpen
        ? BUSINESS_SCHEDULE_ALWAYS_OPEN_RULES
        : (activeRules.length > 0 ? activeRules : DEFAULT_WEEKLY_RULES);
    const segments = expandWeeklyRulesToDaySegments(sourceRules, dow);
    ...
    return {
        segments,
        dayStart: segments[0]?.startTime,
        dayEnd: segments[0]?.endTime,
...
export function resolveBookingWindowDays(value?: number | null) {
    return normalizeBookingWindowDays(value);
}

export function resolveCancellationHours(value?: number | null) {
    return normalizeCancellationHours(value);
}

export function computeMarketplaceResponseDueAt(from = new Date(), slaHours?: number | null): Date {
    const hours = resolveSlaHours(slaHours);
...
export function validateMarketplaceEventLead(
    ...
    if (maxBookingWindowDays != null) {
        const windowError = validateBookingWindowDate(eventDate, maxBookingWindowDays);
...
export function generateMarketplaceTimeSlots(
    ...
    const segments = options?.segments?.length
        ? options.segments
        : [{
            startTime: options?.dayStart ?? MARKETPLACE_DAY_START,
            endTime: options?.dayEnd ?? MARKETPLACE_DAY_END,
        }];

    const merged = new Set<string>();
    for (const segment of segments) {
        for (const slot of generateMarketplaceTimeSlotsForSegment(durationMinutes, dayYmd, existing, options, segment)) {
            merged.add(slot);
        }
    }
    return [...merged].sort();
...
function slotOverlapsBreak(
    ...
    const bStart = minutesFromTime(breakStart);
    const bEnd = minutesFromTime(breakEnd);
    if (bStart == null || bEnd == null || bEnd <= bStart) return false;
    const slotEnd = cursor + durationMinutes;
    return cursor < bEnd && bStart < slotEnd;
}

function generateMarketplaceTimeSlotsForSegment(
    durationMinutes: number,
    dayYmd: string,
    existing: SlotSerenata[],
    options: ProviderGroupSlotOptions | undefined,
    segment: { startTime: string; endTime: string; breakStart?: string | null; breakEnd?: string | null },
): string[] {
    const dayStart = minutesFromTime(segment.startTime) ?? 9 * 60;
    const dayEnd = minutesFromTime(segment.endTime) ?? 23 * 60;
    ...
    for (let cursor = dayStart; cursor + durationMinutes <= dayEnd; cursor += step) {
        ...
        if (slotOverlapsBreak(cursor, durationMinutes, segment.breakStart, segment.breakEnd)) continue;
...
export async function filterProviderGroupsAvailableOnDate(
    ...
    return groups.filter((group) => {
        ...
        const sourceRules = group.alwaysOpen
            ? BUSINESS_SCHEDULE_ALWAYS_OPEN_RULES
            : (activeRules.length > 0 ? activeRules : DEFAULT_WEEKLY_RULES);
        const segments = expandWeeklyRulesToDaySegments(sourceRules, groupDow);
        if (!group.alwaysOpen && activeRules.length > 0 && segments.length === 0) return false;
