...
import {
    ...
    serenataGroupServicePacks,
    serenataGroupServicePromotions,
...
export const providerGroupWriteSchema = z.object({
    ...
    publicEmail: emptyStringToNull,
    websiteUrl: emptyStringToNull,
    instagramUrl: emptyStringToNull,
    facebookUrl: emptyStringToNull,
    linkedinUrl: emptyStringToNull,
    tiktokUrl: emptyStringToNull,
    youtubeUrl: emptyStringToNull,
    twitterUrl: emptyStringToNull,
    ...
    accountKind: z.enum(['individual', 'independent', 'company']).optional(),
    operatorSubtype: z.string().trim().max(40).nullable().optional(),
    operatorSubtypeCustom: z.string().trim().max(160).nullable().optional(),
    ...
    bookingTermsText: z.string().trim().max(12000).nullable().optional(),
...
export const providerAvailabilityPutSchema = z.object({
    ...
    scheduleNote: z.string().trim().max(255).nullable().optional(),
...
const groupServiceWriteBaseSchema = z.object({
    ...
    isPresential: z.boolean().optional(),
...
export const groupServiceWriteSchema = groupServiceWriteBaseSchema.superRefine((data, ctx) => {
    ...
    const isPresential = data.isPresential ?? true;
    if (!isOnline && !isPresential) {
        ctx.addIssue({
            code: 'custom',
            path: ['isPresential'],
...
const groupServicePackWriteBaseSchema = z.object({
    name: z.string().min(2).max(160),
    description: emptyStringToNull,
    imageUrl: emptyStringToNull,
    sessionsCount: z.number().int().min(1).default(1),
    price: z.number().int().min(1000),
    promoPrice: z.number().int().min(1000).nullable().optional(),
    currency: z.string().min(3).max(8).default('CLP'),
...
export const groupServicePackWriteSchema = groupServicePackWriteBaseSchema.superRefine((data, ctx) => {
    if (data.promoPrice != null && data.promoPrice >= data.price) {
        ctx.addIssue({
            code: 'custom',
            path: ['promoPrice'],
...
export const groupServicePackPatchSchema = groupServicePackWriteBaseSchema.partial();
...
const groupServicePromotionWriteBaseSchema = z.object({
...
export const groupServicePromotionWriteSchema = groupServicePromotionWriteBaseSchema.superRefine((data, ctx) => {
...
export const groupServicePromotionPatchSchema = groupServicePromotionWriteBaseSchema.partial();
...
function mapGroupServicePack(row: typeof serenataGroupServicePacks.$inferSelect) {
    return {
        id: row.id,
        providerGroupId: row.providerGroupId,
        name: row.name,
        ...
        promoPrice: row.promoPrice,
        currency: row.currency,
        appliesTo: (row.appliesTo ?? 'all') as 'all' | 'services',
...
function mapGroupServicePromotion(row: typeof serenataGroupServicePromotions.$inferSelect) {
    return {
        id: row.id,
        providerGroupId: row.providerGroupId,
        label: row.label,
...
function isGroupPromotionActiveNow(
    row: Pick<typeof serenataGroupServicePromotions.$inferSelect, 'isActive' | 'startsAt' | 'endsAt'>,
    now = new Date(),
) {
...
function validateGroupPromotionDiscount(
    discountValue: number,
    discountType: 'percent' | 'fixed',
): string | null {
    if (!Number.isFinite(discountValue) || discountValue <= 0) return 'El descuento debe ser mayor a 0';
    if (discountType === 'percent' && discountValue > 100) return 'El porcentaje no puede superar 100';
    return null;
}

async function fetchActivePublicGroupCatalog(groupId: string) {
    const [services, packs, promotions] = await Promise.all([
        db
            .select()
            .from(serenataGroupServices)
            .where(and(eq(serenataGroupServices.providerGroupId, groupId), eq(serenataGroupServices.isActive, true)))
            .orderBy(asc(serenataGroupServices.sortOrder), asc(serenataGroupServices.name)),
        db
            .select()
            .from(serenataGroupServicePacks)
            .where(and(eq(serenataGroupServicePacks.providerGroupId, groupId), eq(serenataGroupServicePacks.isActive, true)))
            .orderBy(asc(serenataGroupServicePacks.sortOrder), asc(serenataGroupServicePacks.name)),
        db
            .select()
            .from(serenataGroupServicePromotions)
            .where(and(eq(serenataGroupServicePromotions.providerGroupId, groupId), eq(serenataGroupServicePromotions.isActive, true)))
            .orderBy(asc(serenataGroupServicePromotions.sortOrder), asc(serenataGroupServicePromotions.label)),
    ]);
    return {
        services: services.map(mapGroupService),
        packs: packs.map(mapGroupServicePack),
        promotions: promotions.filter((row) => isGroupPromotionActiveNow(row)).map(mapGroupServicePromotion),
    };
}

async function resolvePublicMarketplaceGroup(idOrSlug: string) {
    const byId = await db.query.serenataProviderGroups.findFirst({
        where: and(
            eq(serenataProviderGroups.id, idOrSlug),
    ...
    return db.query.serenataProviderGroups.findFirst({
        where: and(
            eq(serenataProviderGroups.slug, idOrSlug),
            eq(serenataProviderGroups.status, 'active'),
            isNotNull(serenataProviderGroups.logoUrl),
            isNotNull(serenataProviderGroups.coverUrl),
        ),
...
export function registerMarketplaceRoutes(app: Hono, deps: MarketplaceDeps) {
    ...
    app.get('/marketplace/groups/:id/services', async (c) => {
        ...
        const group = await resolvePublicMarketplaceGroup(id);
        if (!group) return deps.jsonError(c, 'Grupo no encontrado', 404);
        const services = await db
            .select()
            .from(serenataGroupServices)
            .where(and(eq(serenataGroupServices.providerGroupId, group.id), eq(serenataGroupServices.isActive, true)))
            .orderBy(asc(serenataGroupServices.sortOrder), asc(serenataGroupServices.name));
        return c.json({ ok: true, items: services.map(mapGroupService) });
    });

    app.get('/marketplace/groups/:id/service-packs', async (c) => {
        const group = await resolvePublicMarketplaceGroup(c.req.param('id'));
        if (!group) return deps.jsonError(c, 'Grupo no encontrado', 404);
        const packs = await db
            .select()
            .from(serenataGroupServicePacks)
            .where(and(eq(serenataGroupServicePacks.providerGroupId, group.id), eq(serenataGroupServicePacks.isActive, true)))
            .orderBy(asc(serenataGroupServicePacks.sortOrder), asc(serenataGroupServicePacks.name));
        return c.json({ ok: true, items: packs.map(mapGroupServicePack) });
    });

    app.get('/marketplace/groups/:id/service-promotions', async (c) => {
        const group = await resolvePublicMarketplaceGroup(c.req.param('id'));
        if (!group) return deps.jsonError(c, 'Grupo no encontrado', 404);
        const promotions = await db
            .select()
            .from(serenataGroupServicePromotions)
            .where(and(eq(serenataGroupServicePromotions.providerGroupId, group.id), eq(serenataGroupServicePromotions.isActive, true)))
            .orderBy(asc(serenataGroupServicePromotions.sortOrder), asc(serenataGroupServicePromotions.label));
        return c.json({ ok: true, items: promotions.filter((row) => isGroupPromotionActiveNow(row)).map(mapGroupServicePromotion) });
    });

    app.get('/marketplace/groups/:id/catalog', async (c) => {
        const group = await resolvePublicMarketplaceGroup(c.req.param('id'));
        if (!group) return deps.jsonError(c, 'Grupo no encontrado', 404);
        const catalog = await fetchActivePublicGroupCatalog(group.id);
    ...
    app.patch('/provider-groups/:id', async (c) => {
        ...
        const touchesPaymentMethods = patch.requiresAdvancePayment !== undefined
            || patch.advancePaymentInstructions !== undefined
            || patch.acceptsTransfer !== undefined
            || patch.acceptsMp !== undefined
            || patch.acceptsPaymentLink !== undefined
            || patch.paymentLinkUrl !== undefined
            || patch.bankTransferData !== undefined;
        if (touchesPaymentMethods) {
            const paymentValidationError = getBusinessPaymentMethodsSaveError(
                businessPaymentMethodsFromRecord({
                    requiresAdvancePayment: patch.requiresAdvancePayment ?? group.requiresAdvancePayment,
                    advancePaymentInstructions: patch.advancePaymentInstructions ?? group.advancePaymentInstructions,
                    acceptsTransfer: patch.acceptsTransfer ?? group.acceptsTransfer,
                    acceptsMp: patch.acceptsMp ?? group.acceptsMp,
                    acceptsPaymentLink: patch.acceptsPaymentLink ?? group.acceptsPaymentLink,
                    paymentLinkUrl: patch.paymentLinkUrl ?? group.paymentLinkUrl,
                    bankTransferData: patch.bankTransferData !== undefined
                        ? patch.bankTransferData
                        : (group.bankTransferData as BankTransferData | null),
                }),
            );
            if (paymentValidationError) {
                return deps.jsonError(c, paymentValidationError, 400);
            }
        }
        ...
        const [item] = await db.update(serenataProviderGroups).set({
            ...
            ...(patch.publicEmail !== undefined ? { publicEmail: patch.publicEmail } : {}),
            ...
            ...(patch.twitterUrl !== undefined ? { twitterUrl: patch.twitterUrl } : {}),
            ...
            ...(patch.accountKind !== undefined ? { accountKind: patch.accountKind } : {}),
            ...(patch.operatorSubtype !== undefined ? { operatorSubtype: patch.operatorSubtype } : {}),
            ...(patch.operatorSubtypeCustom !== undefined ? { operatorSubtypeCustom: patch.operatorSubtypeCustom } : {}),
            ...(patch.status !== undefined ? { status: patch.status } : {}),
            ...(patch.slaHours !== undefined ? { slaHours: resolveSlaHours(patch.slaHours) } : {}),
            ...(patch.bookingWindowDays !== undefined ? { bookingWindowDays: resolveBookingWindowDays(patch.bookingWindowDays) } : {}),
            ...(patch.cancellationHours !== undefined ? { cancellationHours: resolveCancellationHours(patch.cancellationHours) } : {}),
            ...(patch.bookingTermsText !== undefined ? { bookingTermsText: patch.bookingTermsText?.trim() || null } : {}),
    ...
    app.get('/provider-groups/:id/availability', async (c) => {
        ...
        return c.json({
            ...
            item: {
                ...
                alwaysOpen: Boolean(access.group.alwaysOpen),
                scheduleNote: access.group.scheduleNote ?? null,
    ...
    app.put('/provider-groups/:id/availability', async (c) => {
        ...
        if (patch.rules) {
            for (const rule of patch.rules) {
                ...
                if (endM === startM) {
                    return deps.jsonError(c, 'La hora de fin debe ser distinta al inicio en cada día.', 400);
                }
            }
        ...
        if (patch.alwaysOpen !== undefined) groupPatch.alwaysOpen = patch.alwaysOpen;
        if (patch.scheduleNote !== undefined) {
            groupPatch.scheduleNote = patch.scheduleNote?.trim() ? patch.scheduleNote.trim().slice(0, 255) : null;
        }
        ...
        return c.json({
            ...
            item: {
                ...
                alwaysOpen: Boolean(group.alwaysOpen),
                scheduleNote: group.scheduleNote ?? null,
                rules: rules.length > 0 ? rules.map(mapAvailabilityRule) : DEFAULT_WEEKLY_RULES,
                blockedSlots: blockedSlots.map(mapBlockedSlot),
            },
        });
    ...
    app.post('/provider-groups/:id/services', async (c) => {
        ...
        const [item] = await db.insert(serenataGroupServices).values({
            ...
            isPresential: parsed.data.isPresential ?? true,
    ...
    app.patch('/provider-groups/:groupId/services/:serviceId', async (c) => {
        ...
        if (patch.imageUrl !== undefined) {
            await cleanupReplacedMediaUrl(existing.imageUrl, patch.imageUrl);
        }
        const nextOnline = patch.isOnline ?? existing.isOnline;
        const nextPresential = patch.isPresential ?? existing.isPresential;
        if (!nextOnline && !nextPresential) {
            return deps.jsonError(c, 'Selecciona al menos una modalidad.', 400);
        }
        const [item] = await db.update(serenataGroupServices).set({
            ...
            ...(patch.isPresential !== undefined ? { isPresential: patch.isPresential } : {}),
    ...
    app.get('/provider-groups/:id/service-packs', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('id'), deps);
        if (!access.ok) return access.response;
        const packs = await db
            .select()
            .from(serenataGroupServicePacks)
            .where(eq(serenataGroupServicePacks.providerGroupId, access.group.id))
            .orderBy(asc(serenataGroupServicePacks.sortOrder), asc(serenataGroupServicePacks.name));
        return c.json({ ok: true, items: packs.map(mapGroupServicePack) });
    });

    app.post('/provider-groups/:id/service-packs', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('id'), deps);
        if (!access.ok) return access.response;
        const parsed = groupServicePackWriteSchema.safeParse(await c.req.json().catch(() => null));
        if (!parsed.success) return deps.jsonError(c, 'Pack inválido');
        if (parsed.data.price <= 0) return deps.jsonError(c, 'El precio debe ser mayor a 0', 400);
        if (parsed.data.sessionsCount < 1) return deps.jsonError(c, 'Debe incluir al menos 1 serenata', 400);
        const [item] = await db.insert(serenataGroupServicePacks).values({
            providerGroupId: access.group.id,
            name: parsed.data.name,
            description: parsed.data.description ?? null,
            imageUrl: parsed.data.imageUrl ?? null,
        ...
        return c.json({ ok: true, item: mapGroupServicePack(item) }, 201);
    });

    app.patch('/provider-groups/:groupId/service-packs/:packId', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('groupId'), deps);
        if (!access.ok) return access.response;
        const parsed = groupServicePackPatchSchema.safeParse(await c.req.json().catch(() => null));
        if (!parsed.success) return deps.jsonError(c, 'Pack inválido');
        const packId = c.req.param('packId');
        const existing = await db.query.serenataGroupServicePacks.findFirst({
            where: and(
                eq(serenataGroupServicePacks.id, packId),
                eq(serenataGroupServicePacks.providerGroupId, access.group.id),
            ),
        });
        if (!existing) return deps.jsonError(c, 'Pack no encontrado', 404);
        const nextPrice = parsed.data.price ?? existing.price;
        const nextPromoPrice = parsed.data.promoPrice !== undefined ? parsed.data.promoPrice : existing.promoPrice;
        if (nextPromoPrice != null && nextPromoPrice >= nextPrice) {
            return deps.jsonError(c, 'El precio oferta debe ser menor al precio normal.', 400);
        }
        if (parsed.data.price != null && parsed.data.price <= 0) {
            return deps.jsonError(c, 'El precio debe ser mayor a 0', 400);
        }
        if (parsed.data.sessionsCount != null && parsed.data.sessionsCount < 1) {
            return deps.jsonError(c, 'Debe incluir al menos 1 serenata', 400);
        }
        const patch = parsed.data;
        if (patch.imageUrl !== undefined) {
            await cleanupReplacedMediaUrl(existing.imageUrl, patch.imageUrl);
        }
        const [item] = await db.update(serenataGroupServicePacks).set({
            ...(patch.name !== undefined ? { name: patch.name } : {}),
            ...(patch.description !== undefined ? { description: patch.description } : {}),
            ...(patch.imageUrl !== undefined ? { imageUrl: patch.imageUrl } : {}),
            ...(patch.sessionsCount !== undefined ? { sessionsCount: patch.sessionsCount } : {}),
            ...(patch.price !== undefined ? { price: patch.price } : {}),
            ...(patch.promoPrice !== undefined ? { promoPrice: patch.promoPrice } : {}),
            ...(patch.currency !== undefined ? { currency: patch.currency } : {}),
            ...
            ...(patch.sortOrder !== undefined ? { sortOrder: patch.sortOrder } : {}),
            updatedAt: new Date(),
        }).where(eq(serenataGroupServicePacks.id, packId)).returning();
        return c.json({ ok: true, item: mapGroupServicePack(item) });
    });

    app.delete('/provider-groups/:groupId/service-packs/:packId', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('groupId'), deps);
        if (!access.ok) return access.response;
        await db.update(serenataGroupServicePacks).set({ isActive: false, updatedAt: new Date() }).where(and(
            eq(serenataGroupServicePacks.id, c.req.param('packId')),
            eq(serenataGroupServicePacks.providerGroupId, access.group.id),
        ));
        return c.json({ ok: true });
    });

    app.get('/provider-groups/:id/service-promotions', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('id'), deps);
        if (!access.ok) return access.response;
        const promotions = await db
            .select()
            .from(serenataGroupServicePromotions)
            .where(eq(serenataGroupServicePromotions.providerGroupId, access.group.id))
            .orderBy(asc(serenataGroupServicePromotions.sortOrder), asc(serenataGroupServicePromotions.label));
        return c.json({ ok: true, items: promotions.map(mapGroupServicePromotion) });
    });

    app.post('/provider-groups/:id/service-promotions', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('id'), deps);
        if (!access.ok) return access.response;
        const parsed = groupServicePromotionWriteSchema.safeParse(await c.req.json().catch(() => null));
        if (!parsed.success) return deps.jsonError(c, 'Promoción inválida');
        const discountError = validateGroupPromotionDiscount(parsed.data.discountValue, parsed.data.discountType);
        if (discountError) return deps.jsonError(c, discountError, 400);
        const [item] = await db.insert(serenataGroupServicePromotions).values({
            providerGroupId: access.group.id,
        ...
        return c.json({ ok: true, item: mapGroupServicePromotion(item) }, 201);
    });

    app.patch('/provider-groups/:groupId/service-promotions/:promoId', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('groupId'), deps);
        if (!access.ok) return access.response;
        const parsed = groupServicePromotionPatchSchema.safeParse(await c.req.json().catch(() => null));
        if (!parsed.success) return deps.jsonError(c, 'Promoción inválida');
        const promoId = c.req.param('promoId');
        const existing = await db.query.serenataGroupServicePromotions.findFirst({
            where: and(
                eq(serenataGroupServicePromotions.id, promoId),
                eq(serenataGroupServicePromotions.providerGroupId, access.group.id),
            ),
        });
        if (!existing) return deps.jsonError(c, 'Promoción no encontrada', 404);
        const nextDiscountType = (parsed.data.discountType ?? existing.discountType ?? 'percent') as 'percent' | 'fixed';
        const nextDiscountValue = parsed.data.discountValue ?? existing.discountValue;
        const discountError = validateGroupPromotionDiscount(nextDiscountValue, nextDiscountType);
        if (discountError) return deps.jsonError(c, discountError, 400);
        const patch = parsed.data;
        const [item] = await db.update(serenataGroupServicePromotions).set({
            ...
            ...(patch.sortOrder !== undefined ? { sortOrder: patch.sortOrder } : {}),
            updatedAt: new Date(),
        }).where(eq(serenataGroupServicePromotions.id, promoId)).returning();
        return c.json({ ok: true, item: mapGroupServicePromotion(item) });
    });

    app.delete('/provider-groups/:groupId/service-promotions/:promoId', async (c) => {
        const user = await deps.authUser(c);
        if (!user) return deps.jsonError(c, 'No autenticado', 401);
        const access = await requireProviderGroupAccess(c, user.id, c.req.param('groupId'), deps);
        if (!access.ok) return access.response;
        await db.update(serenataGroupServicePromotions).set({ isActive: false, updatedAt: new Date() }).where(and(
            eq(serenataGroupServicePromotions.id, c.req.param('promoId')),
            eq(serenataGroupServicePromotions.providerGroupId, access.group.id),
        ));
        return c.json({ ok: true });
    });
...
export async function createMarketplaceSerenata(
    ...
    const bookingTermsText = group.bookingTermsText?.trim() ?? '';
    if (bookingTermsText && payload.policyAgreed !== true) {
        return {
            ok: false as const,
            error: 'Debes leer y aceptar las políticas y condiciones del grupo.',
            status: 400 as const,
        };
    }
    ...
    const [item] = await db.insert(serenatas).values({
        ...
        policyAgreed: bookingTermsText ? payload.policyAgreed === true : false,
        policyAgreedAt: bookingTermsText && payload.policyAgreed === true ? new Date() : null,
    }).returning();
