...
import { mortgageRates } from '../db/schema.js';
...
async function scrapeCMF(): Promise<RateData | null> {
  try {
    ...
    logger.info('[CMF] Intentando obtener datos oficiales');
...
export async function updateMortgageRates(): Promise<void> {
  logger.info('[MortgageRates] Iniciando actualización');
  logger.info('[MortgageRates] Consultando fuentes', { count: RATE_SOURCES.length });
  ...
  try {
    ...
    logger.info('[MortgageRates] Tasas de bancos obtenidas', { count: bankRates.length });
    
    // 3. Calcular estadísticas
    const stats = calculateWeightedAverage(bankRates);
    logger.info('[MortgageRates] Estadísticas calculadas', { average: stats.average, best: stats.best, median: stats.median });
    ...
    logger.info('[MortgageRates] Actualizadas correctamente', {
      standardRate,
      bestMarketRate,
      highestRate,
      subsidyRate,
      bankCount: stats.count,
    });
    ...
    if (activeRates.length > 5) {
      ...
      logger.info('[MortgageRates] Registros antiguos desactivados', { count: toDeactivate.length });
    }
    
  } catch (error) {
    logger.error('[MortgageRates] Error en actualización', error);
    throw error;
  }
}
...
export async function getCurrentRates(): Promise<{
  ...
  try {
    ...
    logger.error('[MortgageRates] Error obteniendo tasas', error);
    return null;
  }
}
