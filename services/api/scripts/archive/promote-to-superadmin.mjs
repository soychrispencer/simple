 ...
 * 
 * Uso:
 *   node scripts/archive/promote-to-superadmin.mjs <email> [role] [admin-email] [admin-password] [api-url]
 * 
 * Ejemplos:
 *   # Versión rápida (requiere estar logueado en SimpleAdmin)
 *   node scripts/archive/promote-to-superadmin.mjs chris@example.com superadmin
 * 
 *   # Versión con credenciales de admin
 *   node scripts/archive/promote-to-superadmin.mjs chris@example.com superadmin admin@example.com password123
 * 
 *   # Con URL personalizada
 *   node scripts/archive/promote-to-superadmin.mjs chris@example.com superadmin admin@example.com password123 http://api.example.com
 */
...
if (!email) {
    ...
    console.error('Uso: node scripts/archive/promote-to-superadmin.mjs <email-a-promover> [role] [admin-email] [admin-password] [api-url]');
    process.exit(1);
}
...
(async () => {
    try {
        ...
        if (listResponse.status === 401) {
            ...
            console.error('     node scripts/archive/promote-to-superadmin.mjs user@email admin@email.com password123');
