...
function findBaselineSeedPath() {
  ...
  const files = fs.readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('_baseline_seed.sql')).sort();
  return files.length ? path.join(MIGRATIONS_DIR, files[files.length - 1]) : null;
}

const filePath = findBaselineSeedPath();
if (!filePath) throw new Error('No se encontró *_baseline_seed.sql en backend/supabase/migrations');
