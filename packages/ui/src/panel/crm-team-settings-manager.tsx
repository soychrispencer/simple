...
import { PanelBlockHeader, PanelChoiceCard, PanelNotice, PanelStatusBadge } from './panel-primitives';
...
function Field({ label, icon, children }: { label: string; icon?: ReactNode; children: ReactNode }) {
    return (
        <div className="space-y-1.5">
            <label className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--fg-secondary)]">
                {icon}
                {label}
            </label>
            {children}
...
function SummaryItem({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex items-center justify-between gap-4 rounded-card border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-sm">
            <span className="text-[var(--fg-secondary)]">{label}</span>
            <span className="text-right font-medium text-[var(--fg)]">{value}</span>
        </div>
    );
}
...
export type CrmTeamSettingsManagerProps = {
    vertical: PublicProfileVertical;
    roleTitlePlaceholder?: string;
};

export function CrmTeamSettingsManager({
    vertical,
    roleTitlePlaceholder = 'Ej: Asesor comercial',
}: CrmTeamSettingsManagerProps) {
    ...
    useEffect(() => {
        let active = true;
        void (async () => {
            const response = await fetchAccountPublicProfileForVertical(vertical);
            if (!active) return;
            if (response?.ok) {
    ...
    const handleSave = async () => {
        ...
        const response = await updateAccountPublicProfileForVertical(vertical, payload);
        setSaving(false);
        if (!response.ok) {
        ...
        return (
            <PanelCard size="lg" className="space-y-5">
                ...
                <div className="space-y-2">
                    <h3 className="text-xl font-semibold text-[var(--fg)]">Equipo y routing está disponible para cuentas pagadas.</h3>
            ...
            <PanelCard size="lg" className="space-y-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-xl font-semibold text-[var(--fg)]">Equipo comercial y routing</h3>
                ...
                <PanelCard size="lg" className="space-y-5">
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                        {ROUTING_OPTIONS.map((option) => (
                            ...
                            >
                                <div className="space-y-2">
                                    <div className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-[var(--bg-muted)] text-[var(--fg-secondary)]">
                                        {option.icon}
                                    </div>
                                    <div>
                                        <p className="text-sm font-semibold text-[var(--fg)]">{option.label}</p>
                                        <p className="text-xs leading-5 text-[var(--fg-secondary)]">{option.description}</p>
                                    </div>
                                </div>
                            </PanelChoiceCard>
                        ))}
                    ...
                    <div className="space-y-3">
                        {form.teamMembers.length === 0 ? (
                            ...
                            form.teamMembers.map((member, index) => (
                                <div key={member.id ?? `${member.name}-${index}`} className="rounded-card border border-[var(--border)] bg-[var(--surface)] p-4">
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="space-y-3">
                                            <div>
                                                <p className="text-sm font-semibold text-[var(--fg)]">{member.name}</p>
                                                <p className="text-xs text-[var(--fg-secondary)]">{member.roleTitle || 'Asesor comercial'}</p>
                                            </div>
                ...
                <PanelCard size="lg" className="space-y-5">
                    ...
                    <div className="grid gap-4">
                        ...
                        <Field label="Cargo o rol" icon={<IconBadgeTm size={14} />}>
                            <input className="form-input" value={teamDraft.roleTitle ?? ''} onChange={(event) => setTeamDraft((current) => ({ ...current, roleTitle: event.target.value }))} placeholder={roleTitlePlaceholder} />
                        </Field>
                        ...
                        <label className="inline-flex items-center gap-2 text-sm text-[var(--fg-secondary)]">
