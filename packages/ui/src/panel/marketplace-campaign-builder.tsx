...
import { PanelActions, PanelSummaryCard } from './panel-display.js';
import { PanelButton, getPanelButtonClassName, getPanelButtonStyle } from './panel-button.js';
import { PanelCard } from './panel-card.js';
import { PanelStepNav, PanelSegmentedToggle } from './panel-navigation.js';
import {
    PanelBlockHeader,
    PanelChoiceCard,
    PanelNotice,
...
export type MarketplaceAdvertisingClient = {
    AD_UPDATE_EVENT: string;
    fetchMyAdCampaigns: () => Promise<{ items: AdCampaign[]; unauthorized?: boolean; error?: string }>;
    createAdCampaign: (input: CreateAdCampaignInput) => Promise<{ ok: boolean; item?: AdCampaign; error?: string }>;
    updateAdCampaignContent: (
        campaignId: string,
        input: UpdateAdCampaignContentInput,
    ) => Promise<{ ok: boolean; item?: AdCampaign; error?: string }>;
    updateAdCampaignLifecycle: (
        campaignId: string,
        lifecycleStatus: 'paused' | 'scheduled' | 'active',
    ) => Promise<{ ok: boolean; error?: string }>;
    deleteAdCampaign: (campaignId: string) => Promise<{ ok: boolean; error?: string }>;
    emitCampaignsUpdated: () => void;
};

export type MarketplaceCampaignBuilderProps = {
    vertical: AdvertisingVertical;
    fetchListingOptions: () => Promise<Array<{ label: string; href: string }>>;
    fetchProfileSlug?: () => Promise<string | null>;
    startAdvertisingCheckout: (input: {
        returnUrl: string;
        campaignId: string;
    ...
    advertising: MarketplaceAdvertisingClient;
};

const MARKETPLACE_BRAND: Record<AdvertisingVertical, string> = {
    autos: 'SimpleAutos',
    propiedades: 'SimplePropiedades',
    serenatas: 'SimpleSerenatas',
    agenda: 'SimpleAgenda',
};

const FORMAT_META: Record<
    AdFormat,
    { label: string; description: string; desktop: [number, number]; mobile: [number, number] }
> = {
    hero: {
...
function renderImageToDataUrl(image: HTMLImageElement, width: number, height: number, quality: number): string {
...
function getListingDestinationCopy(vertical: AdvertisingVertical) {
    const config = getVerticalAdvertisingConfig(vertical);
    if (config.boostTargetType === 'serenata_group') {
        return {
            label: config.copy.boostTargetLabel,
            hint: `Abre tu ${config.copy.boostTargetLabel.toLowerCase()} dentro del marketplace.`,
            selectLabel: `Elige el ${config.copy.boostTargetLabel.toLowerCase()}`,
            summaryLabel: config.copy.boostTargetLabel,
            emptyMessage: `No tienes ${config.copy.boostTargetLabel.toLowerCase()} disponibles para usar como destino.`,
            selectPlaceholder: `Seleccionar ${config.copy.boostTargetLabel.toLowerCase()}`,
            selectEmptyPlaceholder: `Sin ${config.copy.boostTargetLabel.toLowerCase()} disponibles`,
        };
    }
    if (config.boostTargetType === 'operator_profile') {
        return {
            label: config.copy.boostTargetLabel,
            hint: 'Dirige al perfil publicado en el directorio.',
            selectLabel: 'Elige el perfil',
            summaryLabel: config.copy.boostTargetLabel,
            emptyMessage: 'No tienes perfiles disponibles para usar como destino.',
            selectPlaceholder: 'Seleccionar perfil',
            selectEmptyPlaceholder: 'Sin perfiles disponibles',
        };
    }
    return {
        label: 'Publicación interna',
        hint: `Abre una publicación dentro de ${MARKETPLACE_BRAND[vertical]}.`,
        selectLabel: 'Elige la publicación',
        summaryLabel: 'Publicación interna',
        emptyMessage: 'No tienes publicaciones disponibles para usar como destino.',
        selectPlaceholder: 'Seleccionar publicación',
        selectEmptyPlaceholder: 'Sin publicaciones disponibles',
    };
}

export function MarketplaceCampaignBuilder({
    vertical,
    fetchListingOptions,
    fetchProfileSlug,
    ...
    confirmCheckout,
    advertising,
}: MarketplaceCampaignBuilderProps) {
    const {
        AD_UPDATE_EVENT,
        fetchMyAdCampaigns,
        createAdCampaign,
    ...
    const fmt = useMemo(() => createPanelFormatters(), []);
    const searchParams = useSearchParams();
    const config = getVerticalAdvertisingConfig(vertical);
    const theme = getThemePrefix(vertical);
    const listingCopy = getListingDestinationCopy(vertical);
    const defaultInlineSection = (config.adInlinePlacements[0]?.id ?? 'ventas') as AdPlacementSection;

    const formats = useMemo<FormatConfig[]>(() => {
        const pricing = MARKETPLACE_AD_PRICING[vertical];
        return config.adFormats.map((id) => ({
            id,
            ...FORMAT_META[id],
            price: pricing[id],
        }));
    }, [config.adFormats, vertical]);

    const destinations = useMemo<Array<{ id: AdDestinationType; label: string; hint: string; badge?: string }>>(
        () => [
            {
                id: 'none',
                label: 'Sin link',
            ...
            {
                id: 'listing',
                label: listingCopy.label,
                hint: listingCopy.hint,
            },
            {
                id: 'profile',
                label: config.copy.profileDestinationLabel,
                hint: 'Dirige al mini-sitio del anunciante.',
            },
        ],
        [config.copy.profileDestinationLabel, listingCopy.hint, listingCopy.label],
    );

    const inlineSectionOptions = useMemo(
        () =>
            config.adInlinePlacements.map((item) => ({
                id: item.id as AdPlacementSection,
                label: item.label,
            })),
        [config.adInlinePlacements],
    );
    ...
    const [inlinePlacementSection, setInlinePlacementSection] = useState<AdPlacementSection>(defaultInlineSection);
    ...
    const selected = useMemo(() => formats.find((item) => item.id === format) ?? formats[0], [format, formats]);
    ...
    useEffect(() => {
        void (async () => {
            const options = await fetchListingOptions();
    ...
    useEffect(() => {
        if (!fetchProfileSlug) return;
        void (async () => {
            const slug = await fetchProfileSlug();
            if (slug) setProfileSlug((current) => current || slug);
        })();
    }, [fetchProfileSlug]);
    ...
    }, [confirmCheckout, emitCampaignsUpdated, handledPurchaseId, searchParams]);
    ...
    const resetBuilder = () => {
        ...
        setProfileSlug('');
        setInlinePlacementSection(defaultInlineSection);
    ...
    const fillFormFromCampaign = (campaign: AdCampaign, mode: 'edit' | 'duplicate') => {
        const formatConfig = formats.find((item) => item.id === campaign.format) ?? formats[0];
        ...
        setInlinePlacementSection(campaign.placementSection ?? defaultInlineSection);
        setDesktopAsset({
            dataUrl: campaign.desktopImageDataUrl,
            width: formatConfig.desktop[0],
            height: formatConfig.desktop[1],
        ...
        setMobileAsset(
            campaign.mobileImageDataUrl
                ? {
                      ...
                      width: formatConfig.mobile[0],
                      height: formatConfig.mobile[1],
    ...
    const validateStep = (target: Step): string | null => {
        if (target === 0) {
            ...
            if (destinationType === 'listing' && !listingHref.trim()) {
                return listingCopy.emptyMessage;
            }
    ...
    const onUpload = async (device: Device, event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const targetBytes = device === 'desktop' ? (format === 'hero' ? 220_000 : 180_000) : 160_000;
    ...
    const togglePause = (id: string) => {
        ...
        if (target.status === 'paused' && target.format === 'hero') {
            const shouldBeActive =
                new Date(target.startAt).getTime() <= now && new Date(target.endAt).getTime() > now;
            if (shouldBeActive) {
        ...
        void (async () => {
            const result = await updateAdCampaignLifecycle(
                ...
                target.status === 'paused' &&
                    new Date(target.startAt).getTime() <= now &&
                    new Date(target.endAt).getTime() > now
                    ? 'active'
                    : target.status === 'paused'
                      ? 'scheduled'
                      : 'paused',
        ...
        <>
            <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-6">
                <div className={`${theme}-stat-tile`}>
                    <p className="type-caption text-[var(--fg-muted)]">Vigentes</p>
                    <p className="text-xl font-semibold">
                        {counters.totalNotEnded}/{MAX_CAMPAIGNS_TOTAL}
                    </p>
                </div>
                <div className={`${theme}-stat-tile`}>
                    <p className="type-caption text-[var(--fg-muted)]">Hero activas</p>
                    <p className="text-xl font-semibold">
                        {counters.activeHero}/{MAX_ACTIVE_HERO_CAMPAIGNS}
                    </p>
                </div>
                <div className={`${theme}-stat-tile`}>
                    <p className="type-caption text-[var(--fg-muted)]">Programadas</p>
                    <p className="text-xl font-semibold">{counters.scheduled}</p>
                </div>
                <div className={`${theme}-stat-tile`}>
                    <p className="type-caption text-[var(--fg-muted)]">Costo referencial</p>
                    <p className="text-xl font-semibold">${selected.price[duration].toLocaleString('es-CL')}</p>
                </div>
            </div>
            ...
            {isEditing && editingCampaign ? (
                <div className={`mb-4 flex flex-wrap items-center gap-2 rounded-lg border px-3 py-2 ${theme}-subtle-card`}>
            ...
            <PanelCard className="mb-8" size="md">
                {step === 0 ? (
                    <div className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {formats.map((item) => (
                                <PanelChoiceCard
                                    key={item.id}
                                    onClick={() => {
                                        if (!lockCommercialFields) setFormat(item.id);
                                    }}
                                    disabled={lockCommercialFields}
                                    selected={format === item.id}
                                >
                                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-3 ${theme}-icon-tile`}>
                                        {item.id === 'hero' ? (
                                    ...
                                    <p className="type-caption mt-2 text-[var(--fg-secondary)]">
                                        Desktop {item.desktop[0]}x{item.desktop[1]} · Móvil {item.mobile[0]}x
                                        {item.mobile[1]}
                                    </p>
                                </PanelChoiceCard>
                            ))}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {([7, 15, 30] as Duration[]).map((days) => (
                                        <button
                                            key={days}
                                            onClick={() => {
                                                if (!lockCommercialFields) setDuration(days);
                                            }}
                                            disabled={lockCommercialFields}
                                            className={`rounded-button border py-2 text-sm disabled:cursor-not-allowed ${theme}-duration-btn ${duration === days ? `${theme}-duration-btn--active` : ''}`}
                                            style={{ opacity: lockCommercialFields && duration !== days ? 0.6 : 1 }}
                                        >
                                            {days} días
                                        </button>
                                    ))}
                        ...
                        <div className={`${theme}-stat-tile`}>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                <span
                                                    className={`mt-0.5 w-4 h-4 rounded-full border inline-flex items-center justify-center ${theme}-radio-ring ${active ? `${theme}-radio-ring--active` : ''}`}
                                                    aria-hidden
                                                >
                                                    {active ? (
                                                        <span className={`w-2 h-2 rounded-full ${theme}-radio-dot`} />
                                                    ) : null}
                                                </span>
                                                <span className="min-w-0">
                                                    <span className={`text-sm font-medium block ${theme}-text-fg`}>
                                                        {option.label}
                                                    </span>
                                                    <span className={`type-caption block mt-0.5 ${theme}-text-muted`}>
                                                        {option.hint}
                                                    </span>
                                                </span>
                                            </div>
                                            {option.badge ? (
                                                <span
                                                    className={`inline-flex mt-2 rounded-full border px-2 py-0.5 text-[11px] font-medium ${theme}-dest-badge`}
                                                >
                                                    {option.badge}
                                                </span>
                                            ) : null}
                            ...
                            <div className={`mt-3 rounded-lg border px-3 py-2 ${theme}-subtle-box`}>
                            ...
                            {destinationType === 'listing' ? (
                                <div className="mt-3">
                                    <label className="type-caption font-medium mb-1 block text-[var(--fg-secondary)]">
                                        {listingCopy.selectLabel}
                                    </label>
                                    <ModernSelect
                                        value={listingHref}
                                        onChange={setListingHref}
                                        options={listingOptions.map((item) => ({ value: item.href, label: item.label }))}
                                        ariaLabel={listingCopy.selectLabel}
                                        placeholder={
                                            listingOptions.length > 0
                                                ? listingCopy.selectPlaceholder
                                                : listingCopy.selectEmptyPlaceholder
                                        }
                                    />
                                </div>
                            ) : null}
                        ...
                        {format === 'inline' ? (
                            <div className={`${theme}-stat-tile`}>
                                <p className="text-sm font-medium">Sección objetivo del inline</p>
                                <p className="type-caption mt-1 mb-3">
                                    El banner inline solo se mostrará en la sección seleccionada.
                                </p>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    {inlineSectionOptions.map((sectionOption) => {
                                        const active = inlinePlacementSection === sectionOption.id;
                                        return (
                                            <button
                                                key={sectionOption.id}
                                                onClick={() => {
                                                    if (!lockCommercialFields) setInlinePlacementSection(sectionOption.id);
                                                }}
                                                disabled={lockCommercialFields}
                                                className={`rounded-lg border p-2 text-sm disabled:cursor-not-allowed ${theme}-inline-pick ${active ? `${theme}-inline-pick--active` : ''}`}
                                                style={{ opacity: lockCommercialFields && !active ? 0.6 : 1 }}
                                            >
                                                {sectionOption.label}
                                            </button>
                                        );
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div className={`${theme}-stat-tile`}>
                                <p className="text-sm font-medium mb-2">
                                    Imagen desktop ({selected.desktop[0]}x{selected.desktop[1]})
                                </p>
                                <input
                                    type="file"
                                    accept="image/*"
                                    onChange={(event) => void onUpload('desktop', event)}
                                    className="form-input h-auto py-2"
                                />
                                {desktopAsset ? (
                                    <p className={`type-caption mt-2 ${theme}-text-muted`}>
                                        {desktopAsset.name} · {desktopAsset.width}x{desktopAsset.height}
                                    </p>
                                ) : null}
                            </div>
                            <div className={`${theme}-stat-tile`}>
                                <p className="text-sm font-medium mb-2">
                                    Imagen móvil opcional ({selected.mobile[0]}x{selected.mobile[1]})
                                </p>
                                <input
                                    type="file"
                                    accept="image/*"
                                    onChange={(event) => void onUpload('mobile', event)}
                                    className="form-input h-auto py-2"
                                />
                                {mobileAsset ? (
                                    <p className="type-caption mt-2">
                                        {mobileAsset.name} · {mobileAsset.width}x{mobileAsset.height}
                                    </p>
                                ) : null}
                            </div>
                        </div>
                            ...
                            {overlayEnabled ? (
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => setOverlaySubtitle(event.target.value)}
                                    />
                                    <input
                                        className="form-input"
                                        placeholder="Texto botón opcional"
                                        value={overlayCta}
                                        onChange={(event) => setOverlayCta(event.target.value)}
                                    />
                                </div>
                            ) : null}
                        </div>

                        <div className={`rounded-xl border p-3 ${theme}-subtle-box`}>
                            <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                            ...
                            <div
                                className={`rounded-lg border overflow-hidden ${theme}-preview-frame`}
                                style={{
                                    aspectRatio: `${previewSpec[0]} / ${previewSpec[1]}`,
                                    background: previewImage
                                        ? `linear-gradient(to right, rgba(0,0,0,0.45), rgba(0,0,0,0.2)), url(${previewImage}) center / cover no-repeat`
                                        : undefined,
                                }}
                            >
                                <div className={`h-full w-full p-4 md:p-6 flex flex-col justify-end ${alignClass}`}>
                                    {showOverlay ? (
                                        <div className="max-w-xl">
                                            ...
                                            {overlaySubtitle ? (
                                                <p className="text-sm text-white/85 mt-1">{overlaySubtitle}</p>
                                            ) : null}
                                            {overlayCta ? (
                                                <span className="inline-flex mt-3 rounded-md border border-white/50 px-3 py-1 text-xs text-white">
                                                    {overlayCta}
                                                </span>
                                            ) : null}
                                        </div>
                                    ) : (
                                        <span className="text-xs text-white/70">Preview sin texto overlay.</span>
                                    )}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        <div className={`${theme}-stat-tile p-4`}>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <input
                                    type="date"
                                    className="form-input max-w-[220px] disabled:cursor-not-allowed"
                                    value={startDate}
                        ...
                        >
                            ...
                            <Link
                                href="/"
                                className={getPanelButtonClassName({ className: 'mt-2 w-full' })}
                                style={getPanelButtonStyle('secondary')}
                            >
                                Ver portada en inicio
                            </Link>
                        </PanelSummaryCard>
                    </div>
                ) : null}
                ...
                <PanelActions
                    className="mt-6"
                    left={
                        <PanelButton onClick={prevStep} variant="secondary" className={step === 0 ? 'invisible' : ''}>
                            Atrás
                        </PanelButton>
                    }
                    right={
                        step < 2 ? (
                            <PanelButton onClick={nextStep} variant="primary">
            ...
            <PanelCard size="md">
                <PanelBlockHeader
                    ...
                    description="Pausa, reanuda o elimina campañas desde aquí."
                />
                                    ...
                                    >
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2 flex-wrap">
                                                ...
                                                {campaign.paymentStatus !== 'paid' ? (
                                                    <PanelStatusBadge
                                                        label={paymentStatusLabel(campaign.paymentStatus)}
                                                        tone={paymentStatusTone(campaign.paymentStatus)}
                                                    />
                                                ) : null}
                                            </div>
                                            <p className={`type-caption mt-1 ${theme}-text-secondary`}>
                                                {campaign.format.toUpperCase()} · {campaign.durationDays} días ·{' '}
                                                {fmt.dateMedium(campaign.startAt)} - {fmt.dateMedium(campaign.endAt)}
                                            </p>
                                            {campaign.paymentStatus !== 'paid' ? (
                                                <p className="type-caption mt-1 text-[var(--fg-muted)]">
                                                    Esta campaña aún no queda visible públicamente hasta confirmar el
                                                    pago.
                                                </p>
                                            ) : null}
                                            <p className="type-caption mt-1 text-[var(--fg-muted)]">
                                                Destino:{' '}
                                                {campaign.destinationType === 'none'
                                                    ? 'Sin link'
                                                    : campaign.destinationType === 'listing'
                                                      ? campaign.listingHref
                                                      : campaign.destinationType === 'profile'
                                                        ? `/perfil/${campaign.profileSlug ?? ''}`
                                            ...
                                            {href !== '#' ? (
                                                <Link
                                                    ...
                                                    className={`inline-flex mt-2 text-xs font-medium ${theme}-text-secondary`}
                                                >
                                                    Abrir destino
                                                </Link>
                                            ) : null}
                                        ...
                                        <div className="flex flex-wrap items-center gap-2 self-end sm:self-auto">
                                            {campaign.paymentStatus !== 'paid' && campaign.status !== 'ended' ? (
                                                <PanelButton
                                                    onClick={() => void startCampaignCheckout(campaign.id)}
                                                    variant="primary"
                                                    size="sm"
                                                >
