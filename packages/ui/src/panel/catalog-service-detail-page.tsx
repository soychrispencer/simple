...
import {
    fetchPublicOperatorServiceById,
...
export function CatalogServiceDetailPage({ vertical }: { vertical: PublicProfileVertical }) {
    const params = useParams();
    const id = typeof params.id === 'string' ? params.id : '';
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    const [item, setItem] = useState<PublicOperatorServiceItem | null>(null);
    ...
    useEffect(() => {
        ...
        setLoading(true);
        void fetchPublicOperatorServiceById(vertical, id).then((result) => {
            setItem(result.item ?? null);
            setError(result.ok ? '' : (result.error ?? 'Servicio no encontrado'));
            setLoading(false);
        });
    }, [vertical, id]);
    ...
    if (!item) {
        return (
            <div className="container-app space-y-4 py-16">
                <PanelNotice tone="warning">{error || 'Servicio no encontrado'}</PanelNotice>
                <Link href="/servicios" className="inline-flex items-center gap-2 text-sm font-medium text-(--fg)">
    ...
    return (
        <div className="container-app py-8 md:py-12">
            <Link href="/servicios" className="mb-6 inline-flex items-center gap-2 text-sm text-(--fg-muted) hover:text-(--fg)">
                <IconArrowLeft size={16} /> Servicios
            </Link>
            <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
                ...
                <PanelCard size="md" className="space-y-4 p-6">
                    <p className="text-xs font-medium uppercase tracking-wide text-(--fg-muted)">
                        {resolveOperatorServiceCategoryLabel(vertical, item.category)}
                    </p>
