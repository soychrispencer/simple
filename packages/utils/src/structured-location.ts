...
export function getSupportedCountries(): SupportedCountry[] {
    const sorted = [...SUPPORTED_COUNTRIES].sort((a, b) => a.name.localeCompare(b.name, 'es'));
    const chile = sorted.find((item) => item.code === 'CL');
    const rest = sorted.filter((item) => item.code !== 'CL');
    return chile ? [chile, ...rest] : sorted;
}
...
export function formatStructuredLocationLabel(location: StructuredLocation): string {
    ...
    const parts = [
        ...
        location.regionName,
        location.countryCode !== 'CL' ? country : null,
    ].filter(Boolean);
    return parts.join(', ') || country;
}

export function structuredLocationFromPublicProfileFields(input: {
    ...
    if (input.regionId || input.localityId) {
        return normalizeStructuredLocation({
            countryCode: input.countryCode ?? 'CL',
            regionId: input.regionId ?? null,
            regionName: input.region ?? null,
            localityId: input.localityId ?? null,
            localityName: input.city ?? null,
        });
    }
    return structuredLocationFromLegacyNames(input.countryCode ?? 'CL', input.region, input.city);
}

export function publicProfileFieldsFromStructuredLocation(location: StructuredLocation): {
    countryCode: string;
    regionId: string | null;
    localityId: string | null;
    region: string | null;
