...
import { approveModel, rejectModel } from '@/app/actions/catalogAdmin';
...
export default function PendingModelRow({ id, name, brandName, typeSlug, createdBy }: Props) {
	...
	const run = (fn: (modelId: string) => Promise<ActionResult>) => {
		setMessage(null);
		setStatus('idle');
		...
		<div className="border-b border-black/5 dark:border-white/10 pb-2">
			<div className="flex items-center justify-between gap-3 flex-wrap">
				<div className="min-w-[260px]">
					<div className="font-medium text-lighttext dark:text-darktext">
						{name}
						<span className="text-[11px] text-lighttext/70 dark:text-darktext/70">{' '}
							({brandName ?? 'Marca?'} · {typeSlug ?? 'tipo?'})
						</span>
					</div>
				...
				<div className="flex items-center gap-2">
					<Button size="sm" variant="primary" type="button" disabled={pending} onClick={() => run(approveModel)}>
						{pending ? '...' : 'Aprobar'}
					</Button>
					<Button size="sm" variant="ghost" type="button" disabled={pending} onClick={() => run(rejectModel)}>
						{pending ? '...' : 'Rechazar'}
					</Button>
				</div>
			</div>
