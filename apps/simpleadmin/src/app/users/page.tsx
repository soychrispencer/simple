...
function planLabel(sub: SubscriptionRow | undefined): string {
  const key = sub?.subscription_plans?.plan_key;
  const name = sub?.subscription_plans?.name;
  return (name || key || 'Free').toString();
}
...
export default async function AdminUsersPage() {
  ...
  const subsRes = userIds.length
    ? await admin
        .from('subscriptions')
        .select('id,user_id,status,current_period_end,subscription_plans(plan_key,name)')
        .in('user_id', userIds)
        .eq('status', 'active')
    : { data: [], error: null };
  ...
  for (const s of ((subsRes as any).data || []) as any[]) {
    subsByUser.set(s.user_id, s as SubscriptionRow);
  }
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl font-semibold text-lighttext dark:text-darktext">Usuarios y suscripciones</h1>
            <p className="text-sm text-lighttext/70 dark:text-darktext/70">
              Vista administrativa: plan activo y últimos pagos.
            </p>
          </div>
          <a
            href="/"
            className="text-sm text-lighttext/80 dark:text-darktext/80 hover:underline"
          >
            Volver al panel
          </a>
        </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {users.map((u) => {
                  ...
                  const renewal = sub?.current_period_end || null;
                  const isExpired = renewal ? new Date(renewal).getTime() < Date.now() : false;

                  const alerts: string[] = [];
                  if (!sub) alerts.push('Sin suscripción activa');
                  if (sub && isExpired) alerts.push('Suscripción vencida');
                  ...
                  return (
                    <tr key={u.id} className="border-b border-black/5 dark:border-white/10 align-top">
                      ...
                      <td className="py-3 pr-4">
                        ...
                        <div className="text-xs text-lighttext/60 dark:text-darktext/60">{sub?.status || 'free'}</div>
                      </td>
                      <td className="py-3 pr-4">
