...
export async function fetchPropiedadesBusinessSetupStatus(): Promise<MarketplaceBusinessSetupStatus> {
    const [profileResponse, listingsResponse, catalog] = await Promise.all([
        fetchAccountPublicProfile('propiedades'),
    ...
    const steps: BusinessSetupStep[] = [
        ...
        },
        {
            ...
            description: 'Sube tu primer inmueble al marketplace.',
            href: '/panel/publicar',
            complete: hasListing,
        },
    ];
