...
function extractPropertyInfo(meta: string[], title: string, data: PropertyListingCardData): {
    bedrooms: string;
    bathrooms: string;
    surface: string;
    type: string;
    ...
    return { bedrooms, bathrooms, surface, type };
}
...
export default function PropertyListingCard({ data, mode }: Props) {
    ...
    const propertyInfo = extractPropertyInfo(data.meta, data.title, data);
    ...
    const handleShareWhatsApp = async (e: React.MouseEvent) => {
        e.stopPropagation();
        const url = `${window.location.origin}${data.href}`;
        const text = `Mira esta propiedad: ${data.title} - ${data.price}`;
        ...
        >
            ...
            <div className="absolute top-4 left-4 flex flex-col items-start gap-1.5 z-10 max-w-[120px]">
                ...
                <span className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full border border-white/20 bg-black/35 text-white backdrop-blur">
                    {data.variant === 'sale' ? 'Venta' : data.variant === 'rent' ? 'Arriendo' : 'Proyecto'}
                </span>

                {/* Project status si existe */}
                {data.projectStatus && (
            ...
            {/* Sidebar eliminada - ahora todo está en el pie o arriba */}
                ...
                <div className="flex items-start gap-4 mb-2 justify-center">
                    {propertyInfo.type && (
                        <div className="flex flex-col items-center gap-0.5">
                            <IconBuilding size={14} className="text-white/60" />
                            <span className="text-[10px] text-white/90">{propertyInfo.type}</span>
                        </div>
                    )}
                    ...
                    {propertyInfo.surface && (
                        <div className="flex flex-col items-center gap-0.5">
                            <IconRuler size={14} className="text-white/60" />
                            <span className="text-[10px] text-white/90">{propertyInfo.surface}</span>
                        </div>
                    )}
                </div>
