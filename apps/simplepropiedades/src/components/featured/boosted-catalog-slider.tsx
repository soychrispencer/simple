...
export function BoostedCatalogSlider({ section }: { section: CatalogBoostSection }) {
    const [items, setItems] = useState<FeaturedBoostItem[]>([]);
    const [loading, setLoading] = useState(true);
    const sectionMeta = CATALOG_SECTION_META[section];

    const loadItems = useCallback(async () => {
        setLoading(true);
        const featured = await fetchFeaturedBoosted(section, 12);
        setItems(featured);
        setLoading(false);
    }, [section]);
