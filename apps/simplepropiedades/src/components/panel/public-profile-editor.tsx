            ...
            <PanelCard size="lg" className="space-y-5">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-sm" style={{ color: 'var(--fg-secondary)' }}>
                        Activa una suscripción para mostrar una ficha inmobiliaria conectada a tu inventario, datos de contacto, horario y redes sociales.
                    </p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="space-y-6">
            <PanelCard size="lg" className="space-y-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-2">
                        ...
                        <div>
                            <h3 className="text-xl font-semibold" style={{ color: 'var(--fg)' }}>Página pública inmobiliaria</h3>
                            <p className="text-sm" style={{ color: 'var(--fg-secondary)' }}>
                                Conecta tu cuenta pagada con una presencia pública clara, profesional y alineada a tu operación comercial.
                            </p>
                        </div>
                    </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.9fr)]">
                <div className="space-y-6">
                    <PanelCard size="lg" className="space-y-5">
                        <PanelBlockHeader
                            ...
                            description="Define cómo quieres presentarte en SimplePropiedades."
                            className="mb-0"
                        />

                        <div className="grid gap-3 md:grid-cols-3">
                            {[
                                { value: 'individual', label: 'Particular', desc: 'Una persona publicando y atendiendo directamente.', icon: <IconUser size={18} /> },
                                { value: 'independent', label: 'Independiente', desc: 'Corredor o asesor con identidad profesional propia.', icon: <IconBadgeTm size={18} /> },
                                { value: 'company', label: 'Empresa', desc: 'Inmobiliaria, corredora o marca comercial.', icon: <IconBuildingStore size={18} /> },
                            ].map((option) => (
                                <PanelChoiceCard
                                    key={option.value}
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Nombre visible" icon={<IconUser size={14} />}>
                                <input className="form-input" value={form.displayName} onChange={(event) => handleDisplayNameChange(event.target.value)} placeholder="Ej: Estudio Inmobiliario Norte" />
                            </Field>
                            <Field label="Enlace público" icon={<IconLink size={14} />}>
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        placeholder="estudio-inmobiliario"
                                    />
                            ...
                            <Field label="Titular" icon={<IconBadgeTm size={14} />}>
                                <input className="form-input" value={form.headline ?? ''} onChange={(event) => updateForm('headline', event.target.value)} placeholder="Ej: Compra, arriendo y asesoría con foco en atención directa y gestión seria." />
                            </Field>
                            {form.accountKind === 'company' ? (
                                <Field label="Nombre de empresa" icon={<IconBuildingStore size={14} />}>
                                    <input className="form-input" value={form.companyName ?? ''} onChange={(event) => updateForm('companyName', event.target.value)} placeholder="Ej: Inmobiliaria Centro SpA" />
                                </Field>
                            ) : <div />}
                        ...
                        <Field label="Descripción" icon={<IconBadgeTm size={14} />}>
                            <textarea className="form-textarea min-h-[160px]" value={form.bio ?? ''} onChange={(event) => updateForm('bio', event.target.value)} placeholder="Cuenta cómo trabajas, qué tipo de propiedades manejas, cuál es tu enfoque comercial y por qué alguien debería contactarte." />
                        </Field>
                    </PanelCard>
                    ...
                    <PanelCard size="lg" className="space-y-5">
                        <PanelBlockHeader
                            ...
                            description="Estos datos refuerzan confianza y mejoran la calidad del lead."
                            className="mb-0"
                        />

                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Correo público" icon={<IconMail size={14} />}>
                                <input className="form-input" value={form.publicEmail ?? ''} onChange={(event) => updateForm('publicEmail', event.target.value)} placeholder="contacto@tuempresa.cl" />
                            </Field>
                            ...
                            <Field label="Especialidades" icon={<IconAt size={14} />}>
                                <input
                                    ...
                                    placeholder="Departamentos, arriendos, inversión, oficinas"
                                />
                            </Field>
                        </div>
                    </PanelCard>
                ...
                <div className="space-y-6">
                    ...
                    <PanelCard size="lg" className="space-y-5">
                        <div className="flex items-center justify-between gap-3">
                            <PanelBlockHeader
                                ...
                                description="Muestra cuándo atiendes o respondes consultas."
                                className="mb-0"
                            />
                        ...
                        <Field label="Nota adicional" icon={<IconClock size={14} />}>
                            <input className="form-input" value={form.scheduleNote ?? ''} onChange={(event) => updateForm('scheduleNote', event.target.value)} placeholder="Ej: Las visitas se coordinan previamente." />
                        </Field>
                    </PanelCard>
                    ...
                    <PanelCard size="lg" className="space-y-4">
                        <PanelBlockHeader
                            title="Resumen"
                            description="Así se conectará tu página con tu cuenta y tus propiedades."
                            className="mb-0"
                        />
            ...
            <PanelCard size="lg" className="space-y-5">
                <PanelBlockHeader
                    ...
                    description="Agrega asesores o integrantes que quieras mostrar en la ficha pública. La asignación y distribución de leads se administra desde CRM."
                    className="mb-0"
                />
                ...
                <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)]">
                    <div className="space-y-3">
                        {form.teamMembers.length === 0 ? (
                            <PanelNotice tone="neutral">
                                Aún no agregas integrantes del equipo. Puedes dejar la ficha solo con la cuenta principal o sumar asesores visibles.
                            </PanelNotice>
                        ) : (
                    ...
                    <div className="rounded-[24px] border p-5 space-y-4" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
                        ...
                        <Field label="Especialidades" icon={<IconAt size={14} />}>
                            <input
                                ...
                                placeholder="Arriendos, inversión, captación"
                            />
                        </Field>

                        <Field label="Bio breve" icon={<IconNotebook size={14} />}>
                            <textarea className="form-textarea min-h-[120px]" value={teamDraft.bio ?? ''} onChange={(event) => setTeamDraft((current) => ({ ...current, bio: event.target.value }))} placeholder="Describe su foco comercial, territorio o especialidad." />
                        </Field>
