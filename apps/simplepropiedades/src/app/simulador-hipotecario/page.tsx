...
import { PanelBlockHeader, PanelButton, PanelCard, PanelNotice } from '@simple/ui/panel';
...
function getApprovalColor(p: 'high'|'medium'|'low') {
    switch (p) {
        case 'high': return 'marketplace-flow-badge-success';
        case 'medium': return 'marketplace-flow-badge-warning';
        case 'low': return 'marketplace-flow-badge-error';
    }
}
...
export default function SimuladorPage() {
    ...
    const apColor=activeApproval?getApprovalColor(activeApproval):'bg-(--bg-muted)';
            ...
            <div className="marketplace-flow-header">
                <div className="container-app marketplace-flow-header-inner">
                    <div>
                        ...
                        <h1 className="marketplace-flow-title">Simulador hipotecario</h1>
            ...
            <div className="container-app panel-page marketplace-flow-body max-w-4xl mx-auto">
                ...
                {result && (
                    <PanelCard size="lg" className="mb-6 flex items-center gap-5">
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center font-semibold text-lg ${apColor}`}>
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xl font-semibold text-[var(--fg)]">{apLabel}</p>
                ...
                <div className="grid gap-6 lg:grid-cols-5">
                    <div className="lg:col-span-3 space-y-6">
                        <PanelCard size="lg" className="space-y-4">
                            <PanelBlockHeader title="1. Datos del cliente" className="mb-0" />
                            <div className="grid gap-3 sm:grid-cols-2">
                                <div className="sm:col-span-2">
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]">Nombre completo</label>
                                    <input type="text" value={clientName} onChange={e=>setClientName(e.target.value)} className="form-input w-full" placeholder="Ej: Juan Perez" />
                                </div>
                                <div className="sm:col-span-2">
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]">Ingreso líquido mensual</label>
                                    <div className={`marketplace-flow-range-label ${fieldErrors.monthlyIncome ? 'ring-1 ring-[var(--color-error-border)] border-[var(--color-error-border)]' : ''}`}>
                                        <span className="font-semibold">{formatCurrency(parseCLP(monthlyIncome) || 800000)}</span>
                                    </div>
                                    <input type="range" min={800000} max={10000000} step={50000} value={parseCLP(monthlyIncome) || 800000} onChange={e=>setMonthlyIncome(e.target.value)} className="marketplace-flow-range mt-2" />
                                    <div className="flex justify-between text-[10px] mt-1 text-[var(--fg-muted)]">
                                        <span>$800.000</span>
                                        <span>$10.000.000</span>
                                    </div>
                                    {parseCLP(monthlyIncome) < MIN_MONTHLY_INCOME && (
                                        <p className="text-[10px] mt-1 text-(--color-error)">Ingreso inferior al mínimo requerido ({formatCurrency(MIN_MONTHLY_INCOME)})</p>
                                    )}
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]">Edad actual</label>
                                    <input type="number" min={18} max={75} value={age} onChange={e=>setAge(e.target.value)} className={`form-input w-full ${fieldErrors.age || (age && (parseInt(age) < 18 || parseInt(age) > 75)) ? 'ring-1 ring-[var(--color-error-border)] border-[var(--color-error-border)]' : ''}`} />
                                    {age && parseInt(age) < 18 && <p className="text-[10px] mt-1 text-(--color-error)">Edad mínima 18 años</p>}
                                    {age && parseInt(age) > 75 && <p className="text-[10px] mt-1 text-(--color-error)">Edad máxima 75 años</p>}
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <ModernSelect
                                        ...
                                        placeholder="Seleccionar..."
                                        triggerClassName={fieldErrors.employmentYears ? 'ring-1 ring-[var(--color-error-border)] border-[var(--color-error-border)]' : ''}
                                    />
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]">Ahorros disponibles (opcional)</label>
                                    <input type="text" inputMode="numeric" value={availableDownPayment} onChange={e=>setAvailableDownPayment(formatCLP(e.target.value))} className="form-input w-full" placeholder="Ej: 5.000.000" />
                                </div>
                            </div>
                        </PanelCard>
                        <PanelCard size="lg" className="space-y-4">
                            <PanelBlockHeader title="2. Deudas actuales mensuales" className="mb-0" />
                            <div className="grid gap-3 sm:grid-cols-3">
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]" title={DEBT_TYPES_INFO.dividendoHipotecario.bankTreatment}>
                                        Dividendo hipotecario ℹ
                                    </label>
                                    <input type="text" inputMode="numeric" value={debts.dividendoHipotecario} onChange={e=>setDebts(p=>({...p,dividendoHipotecario:formatCLP(e.target.value)}))} className="form-input w-full" placeholder="Ej: 0" />
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]" title={DEBT_TYPES_INFO.creditoConsumo.bankTreatment}>
                                        Crédito de consumo ℹ
                                    </label>
                                    <input type="text" inputMode="numeric" value={debts.creditoConsumo} onChange={e=>setDebts(p=>({...p,creditoConsumo:formatCLP(e.target.value)}))} className="form-input w-full" placeholder="Ej: 0" />
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]" title={DEBT_TYPES_INFO.tarjetaCredito.bankTreatment}>
                                        Límite tarjeta crédito ℹ
                                    </label>
                                    <input type="text" inputMode="numeric" value={debts.tarjetaCredito} onChange={e=>setDebts(p=>({...p,tarjetaCredito:formatCLP(e.target.value)}))} className="form-input w-full" placeholder="Ej: 5.000.000" />
                                    <p className="text-[10px] mt-0.5 text-[var(--fg-muted)]">Se considera {(DEBT_TYPE_FACTORS.tarjetaCredito * 100).toFixed(0)}% del límite</p>
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]" title={DEBT_TYPES_INFO.lineaCredito.bankTreatment}>
                                        Límite línea crédito ℹ
                                    </label>
                                    <input type="text" inputMode="numeric" value={debts.lineaCredito} onChange={e=>setDebts(p=>({...p,lineaCredito:formatCLP(e.target.value)}))} className="form-input w-full" placeholder="Ej: 10.000.000" />
                                    <p className="text-[10px] mt-0.5 text-[var(--fg-muted)]">Se considera {(DEBT_TYPE_FACTORS.lineaCredito * 100).toFixed(0)}% del límite</p>
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]" title={DEBT_TYPES_INFO.creditoAutomotriz.bankTreatment}>
                                        Crédito automotriz ℹ
                                    </label>
                                    <input type="text" inputMode="numeric" value={debts.creditoAutomotriz} onChange={e=>setDebts(p=>({...p,creditoAutomotriz:formatCLP(e.target.value)}))} className="form-input w-full" placeholder="Ej: 0" />
                                </div>
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]" title={DEBT_TYPES_INFO.otraDeuda.bankTreatment}>
                                        Otra deuda ℹ
                                    </label>
                                    <input type="text" inputMode="numeric" value={debts.otraDeuda} onChange={e=>setDebts(p=>({...p,otraDeuda:formatCLP(e.target.value)}))} className="form-input w-full" placeholder="Ej: 0" />
                                </div>
                            </div>
                            <div className="panel-surface-subtle mt-3 p-3 space-y-1">
                        ...
                        <PanelCard size="lg" className="space-y-4">
                            <PanelBlockHeader title="3. Condiciones del crédito" className="mb-0" />
                            <div className="grid gap-3 sm:grid-cols-2">
                                <div>
                                    <label className="text-xs font-medium mb-1 block text-[var(--fg-muted)]">Tasa anual (%)</label>
                                    <input type="number" min={0} step="0.01" value={annualRate} onChange={e=>setAnnualRate(e.target.value)} className="form-input w-full" />
                                ...
                                <div>
                                    ...
                                    <input type="number" min={0} max={90} value={bankPercentage} onChange={e=>setBankPercentage(e.target.value)} className={`form-input w-full ${(parseFloat(bankPercentage)||80) <= 0 || (parseFloat(bankPercentage)||80) > 90 ? 'ring-1 ring-[var(--color-error-border)] border-[var(--color-error-border)]' : ''}`} />
                                    {((parseFloat(bankPercentage)||80) <= 0 || (parseFloat(bankPercentage)||80) > 90) && <p className="text-[10px] mt-1 text-(--color-error)">Debe ser entre 1 y 90</p>}
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <ModernSelect
                                        ...
                                        placeholder="Auto"
                                        triggerClassName={parseInt(age||'35')+(customLoanYears?parseInt(customLoanYears):roundToStandardTerm(Math.max(5,Math.min(30,75-(parseInt(age)||35)))))>75 ? 'ring-1 ring-[var(--color-error-border)] border-[var(--color-error-border)]' : ''}
                                    />
                        ...
                        {validationErrors.length > 0 && (
                            <PanelNotice tone="error">
                                <p className="font-semibold mb-1">Faltan datos requeridos:</p>
                        ...
                        >
                            {isCalculating ? 'Calculando...' : hasCalculated ? 'Actualizar cálculo' : 'Calcular'}
                        </PanelButton>
                    </div>
                    <div className="lg:col-span-2 space-y-6">
                        {result?(
                            <>
                                ...
                                <PanelCard size="lg" className="overflow-hidden">
                                    <PanelBlockHeader
                                        title="Propiedad máxima que puedes comprar"
                                    ...
                                    <div className="marketplace-flow-segment-track">
                                        <button
                                            type="button"
                                            onClick={()=>setScenarioTab('recommended')}
                                            className={`marketplace-flow-segment-btn ${scenarioTab==='recommended' ? 'marketplace-flow-segment-btn--active' : 'text-[var(--fg-muted)] hover:text-[var(--fg)] hover:bg-[var(--bg)]'}`}
                                        >
                                            Recomendado
                                        </button>
                                        <button
                                            ...
                                            className={`marketplace-flow-segment-btn ${scenarioTab==='limit' ? 'marketplace-flow-segment-btn--limit' : 'text-[var(--fg-muted)] hover:text-[var(--fg)] hover:bg-[var(--bg)]'}`}
                                        >
                                    ...
                                    <div className="flex items-center justify-center gap-2 pb-3">
                                        ...
                                        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${result.clientSegment.name === 'Estándar' ? 'marketplace-flow-pill' : result.clientSegment.name === 'Premium' ? 'marketplace-flow-badge-warning' : 'marketplace-flow-accent-pill'}`}>
                                    ...
                                    <div className="pb-6 text-center">
                                        {activeScenario.propertyValue > 0 ? (
                                            <>
                                                <p className="text-5xl font-semibold text-[var(--fg)] tracking-tight">{formatUF(activeScenario.propertyValue,ufValue)}</p>
                                                <p className="text-sm text-[var(--fg-muted)] mt-2 font-medium">{formatCurrency(activeScenario.propertyValue)}</p>
                                            </>
                                        ) : (
                                            <>
                                                <p className="text-5xl font-semibold text-[var(--fg-muted)]">—</p>
                                ...
                                <PanelCard size="lg" className="text-center marketplace-flow-highlight">
                                    <p className="text-[11px] uppercase font-semibold text-(--color-success) mb-1">Cuota mensual a pagar</p>
                                    <p className="text-2xl font-semibold text-[var(--fg)]">{formatCurrency(activeScenario.monthlyPayment)}</p>
                                ...
                                <PanelCard size="lg">
                                    <PanelBlockHeader
                                        ...
                                        className="mb-2"
                                        actions={(
                                            <span className={`text-[11px] font-semibold ${activeScenario.dtiPostRatio > 40 ? 'text-[var(--color-error)]' : activeScenario.dtiPostRatio > 33 ? 'text-[var(--color-warning)]' : 'text-[var(--color-success)]'}`}>
                                    ...
                                    <div className="relative w-full rounded-full h-3 bg-[var(--bg-subtle)] overflow-hidden">
                                        <div className="absolute top-0 left-0 h-full w-[62.5%] marketplace-flow-dti-band--good" />
                                        <div className="absolute top-0 left-[62.5%] h-full w-[20%] marketplace-flow-dti-band--warn" />
                                        <div className="absolute top-0 left-[82.5%] h-full w-[17.5%] opacity-30 bg-gradient-to-r from-[color-mix(in_oklab,var(--color-error)_45%,transparent)] to-[color-mix(in_oklab,var(--color-error)_55%,transparent)]" />
                                        {/* Marcador DTI actual */}
                                        <div 
                                    ...
                                    <div className="flex justify-between text-[9px] mt-1.5 text-[var(--fg-muted)]">
                                        <span>0%</span>
                                        <span className="text-(--color-success)">25% ideal</span>
                                        <span className="text-(--color-warning)">33% límite</span>
                                        <span>40%</span>
                                    </div>
                                </PanelCard>

                                <PanelCard size="lg">
                                    <PanelBlockHeader title="Mejor opción para este perfil" className="mb-2" />
                                    {bestBank.status === 'likely' ? (
                                        <div className="flex items-center gap-3 p-3 rounded-lg bg-(--color-success-subtle) border border-(--color-success-border)">
                                            <div className="w-10 h-10 rounded-full bg-(--color-success) flex items-center justify-center text-(--accent-contrast)">
                                                <IconCheck size={20} />
                                            </div>
                                            <div>
                                                <p className="font-semibold text-(--color-success)">{bestBank.name}</p>
                                                <p className="text-xs text-(--color-success)">Probabilidad alta de aprobación</p>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="marketplace-flow-bank-highlight">
                                            <div className="marketplace-flow-bank-highlight__icon">
                                                <IconAlertTriangle size={20} />
                                            </div>
                                            <div>
                                                <p className="marketplace-flow-bank-highlight__name">{bestBank.name}</p>
                                                <p className="text-xs text-(--color-warning)">Requiere revisión adicional</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="mt-2 pt-2 border-t border-[var(--border)]">
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {banks.filter(b => b.name !== bestBank.name).map(b => (
                                                <span 
                                                    key={b.name} 
                                                    className={`text-[9px] px-1.5 py-0.5 rounded ${b.status==='likely' ? 'bg-(--color-success-subtle) text-(--color-success)' : b.status==='review' ? 'bg-(--color-warning-subtle) text-(--color-warning)' : 'bg-(--color-error-subtle) text-(--color-error)'}`}
                                                >
                                ...
                                {parseCLP(availableDownPayment) > 0 && (
                                    <PanelCard size="sm" tone="subtle" className="space-y-1">
                                ...
                                <PanelButton type="button" variant="secondary" className="w-full justify-center" onClick={()=>setShowAdvanced(!showAdvanced)}>
                                    {showAdvanced?<IconChevronUp size={14}/>:<IconChevronDown size={14}/>}
                                    {showAdvanced?'Ocultar resumen':'Ver resumen completo'}
                                </PanelButton>

                                {showAdvanced&&(
                                    <PanelCard size="lg" className="space-y-4 text-sm">
                                        {activeReason && dismissedReason !== activeReason && (
                                            <div className="p-2 rounded-lg border text-[10px] bg-red-500/5 border-red-500 text-(--color-error) flex justify-between items-start gap-2">
                                ...
                                <PanelButton type="button" variant="secondary" className="w-full justify-center" onClick={handleReset}>
                                    <IconRotate size={18} />
                                    Nueva simulación
                                </PanelButton>
                                <PanelButton type="button" variant="secondary" className="w-full justify-center" onClick={handleDownload}>
                            ...
                            <PanelCard size="lg" className="text-center">
                                <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[var(--bg-subtle)]">
                                    <IconCalculator size={24} className="text-[var(--fg-muted)]" />
                                </div>
                                <p className="text-sm font-medium text-[var(--fg-muted)]">Ingresa los datos y calcula</p>
                                <p className="mt-1 text-xs text-[var(--fg-muted)]">Los resultados aparecerán aquí al calcular</p>
                            </PanelCard>
                        )}
                    </div>
                </div>
