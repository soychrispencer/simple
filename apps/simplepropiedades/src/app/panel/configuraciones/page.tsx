...
import { Button, PanelPageLayout, useSupabase, useToast } from "@simple/ui";
...
export default function ConfiguracionesPage() {
  ...
  const loadInstagramHistory = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/instagram/history?vertical=properties&limit=10", {
  ...
  React.useEffect(() => {
    ...
    if (error === "instagram") {
      const reasonMap: Record<string, string> = {
        ...
        not_logged_in: "Debes iniciar sesión antes de conectar Instagram.",
