...
export default function CRMPage() {
    ...
    useEffect(() => {
        ...
        void (async () => {
            const [services, listingLeads, columns] = await Promise.all([
                fetchCrmLeads('propiedades'),
                fetchCrmListingLeads('propiedades'),
                fetchCrmPipelineColumns('propiedades'),
            ]);
            if (!active) return;
    ...
    useEffect(() => {
        ...
        void (async () => {
            if (tab === 'publicaciones') {
                const detail = await fetchCrmListingLeadDetail(selectedId, 'propiedades');
                if (!active) return;
                setListingDetail(detail);
                setServiceDetail(null);
                if (detail) hydrate(detail.item);
            } else {
                const detail = await fetchCrmLeadDetail(selectedId, 'propiedades');
                if (!active) return;
    ...
    async function refreshListingData(nextColumns?: CrmPipelineColumn[]) {
        const [listingLeads, columns] = await Promise.all([
            fetchCrmListingLeads('propiedades'),
            nextColumns ? Promise.resolve(nextColumns) : fetchCrmPipelineColumns('propiedades'),
        ]);
        setListingItems(listingLeads);
    ...
    async function refreshLeadDetail(leadId: string) {
        if (tab === 'publicaciones') {
            const detail = await fetchCrmListingLeadDetail(leadId, 'propiedades');
            setListingDetail(detail);
            if (detail) hydrate(detail.item);
            return;
        }

        const detail = await fetchCrmLeadDetail(leadId, 'propiedades');
        setServiceDetail(detail);
        if (detail) hydrate(detail.item);
    }
    ...
    async function handleStageChange(leadId: string, nextStatus: CrmLeadStatus) {
        ...
        if (tab === 'publicaciones') {
            ...
            const result = await updateCrmLead(leadId, 'propiedades', { status: nextStatus });
            if (!result.ok || !result.item) {
                setUpdatingLeadId(null);
                setDragLeadId(null);
                setDropStage(null);
    ...
    async function handleCreatePipelineColumn(event: FormEvent<HTMLFormElement>) {
        ...
        const result = await createCrmPipelineColumn('propiedades', {
    ...
    async function handleSavePipelineColumn(columnId: string) {
        ...
        setColumnSaving(true);
        setColumnError(null);
        const result = await updateCrmPipelineColumn(columnId, 'propiedades', {
    ...
    async function handleReorderPipelineColumn(columnId: string, direction: 'left' | 'right') {
        ...
        setColumnSaving(true);
        setColumnError(null);
        const result = await reorderCrmPipelineColumns('propiedades', nextColumns.map((column) => column.id));
        setColumnSaving(false);
    ...
    async function handleDeletePipelineColumn(columnId: string) {
        setColumnSaving(true);
        setColumnError(null);
        const result = await deleteCrmPipelineColumn(columnId, 'propiedades');
        setColumnSaving(false);
    ...
    async function handleQuickAction(lead: AnyLead, action: CrmLeadQuickAction) {
        ...
        const result = 'listing' in lead
            ? await runCrmListingLeadQuickAction(lead.id, 'propiedades', action)
            : await runCrmLeadQuickAction(lead.id, 'propiedades', action);
    ...
    async function handleSave() {
        ...
        if (tab === 'publicaciones') {
            ...
            const result = await updateCrmLead(selectedId, 'propiedades', {
                ...payload,
                assignedToUserId: assignment.assignedToUserId,
            });
            setSaving(false);
    ...
    async function handleAddNote(event: FormEvent<HTMLFormElement>) {
        ...
        if (tab === 'publicaciones') {
            const result = await addCrmListingLeadNote(selectedId, 'propiedades', noteBody.trim());
            setAddingNote(false);
            if (!result.ok || !result.item) {
            ...
            const result = await addCrmLeadNote(selectedId, 'propiedades', noteBody.trim());
            setAddingNote(false);
            if (!result.ok || !result.item) {
                setError(result.error || 'No pudimos guardar la nota.');
                return;
        ...
        <div className="container-app panel-page py-4 lg:py-8">
                    ...
                    {activeLoading ? (
                        ...
                        <PanelEmptyState
                            ...
                            description={tab === 'publicaciones'
                                ? 'Cuando alguien consulte una propiedad aparecerá aquí.'
            ...
            <LeadDetailModal open={detailVisible} onClose={closeLeadDetail}>
                        ...
                        {detailLoading || !detail ? (
                            ...
                            <LeadAccordionSection title="Gestión comercial" subtitle="Estado, asignación y próxima acción." defaultOpen>
                                <div className="grid gap-3 sm:grid-cols-2">
                                    {'listing' in detail.item ? (
                                    ...
                                    <label className="space-y-1.5">
                                        ...
                                        <input className="form-input h-10 text-sm" value={tagsValue} onChange={(event) => setTagsValue(event.target.value)} placeholder="ej. inversionista, cierre rápido" />
                                    </label>
                                    <label className="space-y-1.5 sm:col-span-2">
                                        <span className="text-xs font-medium crm-label-muted">Motivo de cierre</span>
                                        <input className="form-input h-10 text-sm" value={closeReasonValue} onChange={(event) => setCloseReasonValue(event.target.value)} placeholder="ej. arrendó por fuera de la plataforma" />
                                    </label>
                                    <label className="space-y-1.5 sm:col-span-2">
                                        <span className="text-xs font-medium crm-label-muted">Próxima tarea</span>
                                        <input className="form-input h-10 text-sm" value={nextTaskTitle} onChange={(event) => setNextTaskTitle(event.target.value)} placeholder="Ej. Llamar y coordinar visita" />
                                    </label>
                                    <label className="space-y-1.5">
                            ...
                            <LeadAccordionSection title="Notas internas" subtitle="Registro interno del seguimiento comercial.">
                                <form className="space-y-3" onSubmit={(event) => void handleAddNote(event)}>
                                    <textarea className="form-textarea min-h-28 text-sm" value={noteBody} onChange={(event) => setNoteBody(event.target.value)} placeholder="Ej. Cliente pidió visita para este fin de semana." />
                                    <div className="flex items-center justify-between gap-3">
...
function LeadDetailModal(props: { open: boolean; onClose: () => void; children: ReactNode }) {
    return (
                    ...
                    <form className="mt-5 rounded-card border p-4 crm-subtle-card" onSubmit={props.onCreate}>
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_180px_auto]">
                            <input
                                ...
                                placeholder="Ej. Visita agendada"
                            />
                            <select
