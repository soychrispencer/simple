...
export function isAgendaTrialPeriod(profile: { plan: string; planExpiresAt: string | null }): boolean {
    return profile.plan === 'free' && hasAgendaFullAccess(profile);
}
...
export type AgendaProfile = {
    ...
    accountKind?: 'individual' | 'independent' | 'company' | null;
...
export type PreconsultFieldType = AgendaPreconsultFieldType;
export type PreconsultField = AgendaPreconsultField;
...
export type AgendaService = {
    ...
    sessionStatus?: AgendaGroupSessionStatus | null;
...
export async function fetchAgendaService(id: string): Promise<{ service: AgendaService; attendees: AgendaGroupAttendee[] } | null> {
    try {
        const data = await apiFetch<{ ok: boolean; service: AgendaService; attendees: AgendaGroupAttendee[] }>(`/api/agenda/services/${id}`);
        return { service: data.service, attendees: data.attendees ?? [] };
    } catch {
        return null;
    }
}
...
export async function fetchAgendaAvailability(): Promise<{
...
export async function replaceAgendaAvailability(
    rules: Partial<AvailabilityRule>[],
    options?: { alwaysOpen?: boolean; scheduleNote?: string | null },
): Promise<{
    ok: boolean;
    rules?: AvailabilityRule[];
    alwaysOpen?: boolean;
    ...
    return agendaMutation('/api/agenda/availability', {
        method: 'PUT',
...
export type { AgendaGroupAttendeeStatus, AgendaGroupSessionStatus } from '@simple/utils';
...
export type AgendaGroupAttendee = {
    ...
    clientEmail: string | null;
    clientPhone: string | null;
    status: AgendaGroupAttendeeStatus;
    pricePaid: string | null;
    paidAt: string | null;
    notes: string | null;
    registeredAt: string;
...
export async function addServiceAttendee(serviceId: string, body: GroupAttendeeInput): Promise<{ ok: boolean; error?: string; attendee?: AgendaGroupAttendee }> {
    try {
        return await apiFetch(`/api/agenda/services/${serviceId}/attendees`, { method: 'POST', body: JSON.stringify(body) });
    } catch (err) {
        return { ok: false, error: err instanceof Error ? err.message : 'Error desconocido' };
    }
}

export async function patchGroupAttendee(id: string, body: Partial<GroupAttendeeInput> & { status?: AgendaGroupAttendeeStatus }): Promise<{ ok: boolean; error?: string; attendee?: AgendaGroupAttendee }> {
    try {
