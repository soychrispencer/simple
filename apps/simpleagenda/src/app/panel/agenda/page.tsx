...
function defaultAppointmentModality(service: AgendaService | undefined): 'online' | 'presential' {
    if (!service) return 'online';
    return resolveBookingModality(service) ?? 'online';
}

function appointmentModalityOptions(service: AgendaService | undefined): Array<'online' | 'presential'> {
    if (!service) return ['online', 'presential'];
    const options: Array<'online' | 'presential'> = [];
    if (service.isOnline) options.push('online');
    if (service.isPresential) options.push('presential');
...
export default function AgendaPage() {
    ...
    const groupEventsInView = useMemo(() => {
        const wEnd = addDays(weekStart, 6); wEnd.setHours(23, 59, 59, 999);
        const dayEnd = new Date(dayDate); dayEnd.setHours(23, 59, 59, 999);
        const from =
            view === 'month' ? getMonthStart(monthDate).toISOString() :
        ...
        return services.filter((s) => {
            if (s.kind !== 'group_event' || !s.startsAt || s.sessionStatus === 'cancelled') return false;
            const t = new Date(s.startsAt).getTime();
            return t >= fromMs && t <= toMs;
        });
    }, [services, view, weekStart, monthDate, dayDate]);

    const groupByDay: Record<string, AgendaService[]> = {};
    for (const gs of groupEventsInView) {
        if (!gs.startsAt) continue;
    ...
    const handleCreateOpen = (date?: Date) => {
        ...
        setForm({
            ...
            modality: defaultAppointmentModality(defaultService),
            location: '',
    ...
    const handleServiceChange = (serviceId: string) => {
        ...
        setForm((prev) => ({
            ...
            modality: defaultAppointmentModality(svc),
            location: svc && !svc.isPresential ? '' : prev.location,
        }));
    };
                            ...
                            return (
                                ...
                                >
                                    <p className="text-xs font-semibold truncate flex items-center gap-1">
                                        <IconUsersGroup size={11} /> {formatTime(gs.startsAt!)} · {gs.name}
                                    </p>
                                    {height > 36 && (
                                ...
                                >
                                    ...
                                    {loading ? null : combined.slice(0, 3).map((item) => item.kind === 'group' ? (
                                        ...
                                        >
                                            ...
                                            {profile ? formatTimeTz(item.data.startsAt!, profile.timezone) : formatTime(item.data.startsAt!)} {item.data.name}
                                        </button>
                                    ) : (() => {
                ...
                <Modal title="Nueva cita" onClose={() => { setShowCreate(false); setClientSearch(''); setClientDropOpen(false); }}>
                    <div className="flex flex-col gap-4">
                        ...
                        {(() => {
                            ...
                            const modalityOptions = appointmentModalityOptions(selectedService);
                            if (modalityOptions.length <= 1) return null;
                            return (
                        <div className="flex flex-col gap-1.5">
