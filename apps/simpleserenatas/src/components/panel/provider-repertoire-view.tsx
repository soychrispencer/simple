...
export function ProviderRepertoireView({ refresh }: { refresh: () => Promise<void> }) {
    const { group: mariachi, loading, error, refresh: refreshAll, ensureGroup } = useProviderGroupScope(refresh);
    ...
    async function savePending() {
        const activeGroup = mariachi ?? await ensureGroup();
        if (!activeGroup) {
            setFormStatus({ loading: false, error: 'No pudimos guardar las canciones.', ok: null });
            return;
        }
        if (pending.length === 0) {
    ...
    async function uploadScore(instrument: string, file: File) {
        ...
        await serenatasApi.saveRepertoireSongScore(activeGroup.id, scoreSong.id, instrument, {
            storageUrl: uploaded.url,
            originalFilename: file.name,
        });
        const response = await serenatasApi.repertoireSongScores(activeGroup.id, scoreSong.id);
        setScores(response.ok ? response.items : []);
