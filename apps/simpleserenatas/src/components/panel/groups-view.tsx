...
export function GroupsView({
    ...
    const { group: activeMariachi, loading: mariachisLoading, error: mariachisError, refresh: refreshMariachis, ensureGroup } = useProviderGroupScope(refresh);
    const [createMariachi, setCreateMariachi] = useState<ProviderGroup | null>(null);
    ...
    }, [activeMariachi?.id, mariachisLoading, mariachisError, loadMusicianGroups, loadProviderMembers, loadProviderMemberInvites]);

    async function openCreateGroup() {
        setActionError(null);
        let mariachi: ProviderGroup | null = activeMariachi;
        if (!mariachi) {
            mariachi = await ensureGroup();
            if (!mariachi) {
                setActionError('No pudimos preparar los datos del mariachi. Guarda la sección Datos e intenta de nuevo.');
                return;
            }
        }
        setCreateMariachi(mariachi);
        setCreateOpen(true);
    }
    ...
    if (mariachisLoading && !activeMariachi) {
        return (
    ...
    const groupLabel = activeMariachi?.name?.trim() || 'tu mariachi';

    return (
        <div className="w-full">
            <PanelBlockHeader
                title={GRUPOS_TAB_LABEL}
                description={`Grupos de músicos de ${groupLabel}. Úsalos al conformar una serenata en Solicitudes.`}
            />
            ...
            {createOpen && createMariachi ? (
                <CreateMusicianGroupModal
                    mariachi={createMariachi}
                    onClose={() => {
                        setCreateOpen(false);
                        setCreateMariachi(null);
                    }}
