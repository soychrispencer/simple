...
export function PublicMariachiPage({
    ...
    const { group, services, packs, promotions, loading, error } = useMarketplaceGroup(slug, { fallbackData: initialData });
        ...
        <ScreenShell>
            ...
            <div className="mx-auto min-w-0 w-full max-w-5xl overflow-x-hidden px-3 py-4 pb-8 sm:px-6 sm:py-6 lg:py-8">
                {loading ? (
                    ...
                    <MariachiProfileContent group={group} services={services} packs={packs} promotions={promotions} />
                )}
            </div>
        </ScreenShell>
    );
