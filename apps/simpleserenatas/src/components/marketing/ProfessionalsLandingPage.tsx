  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, var(--surface) 0%, var(--bg) 420px)' }}>
      <nav className="flex lg:hidden items-center justify-between px-4 py-3 border-b bg-[var(--surface)]/90 backdrop-blur-sm" style={{ borderColor: 'var(--border)' }}>
        <Link href="/" className="group shrink-0" aria-label="Inicio cliente SimpleSerenatas">
      ...
      <nav className="hidden lg:flex items-center justify-between px-8 py-4 border-b bg-[var(--surface)]/90 backdrop-blur-sm" style={{ borderColor: 'var(--border)' }}>
        ...
        <div className="flex items-center gap-4">
          <Link href="/" className="text-sm font-medium transition-opacity hover:opacity-80" style={{ color: 'var(--fg-secondary)' }}>
            Soy cliente →
          </Link>
          <Link
            href="/auth/login"
            ...
            <div className="hidden lg:block relative">
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 16px 28px color-mix(in oklab, var(--fg) 18%, transparent)',
                }}
              >
      ...
      <section className="py-20 px-6 lg:px-8" style={{ background: 'var(--bg-subtle)' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <StepCard number="1" title="Crea tu perfil" description="Regístrate y completa tu información como músico de mariachi" />
            <StepCard number="2" title="Marca tu disponibilidad" description="Indica cuándo estás disponible para recibir serenatas" />
            <StepCard number="3" title="Recibe solicitudes" description="Empieza a recibir y gestionar serenatas en tu zona" />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 lg:px-8" style={{ background: 'var(--surface)' }}>
        <div className="max-w-5xl mx-auto">
          <div className="rounded-3xl p-8 lg:p-12 text-center" style={{ background: 'var(--accent)', color: 'white' }}>
            <h2 className="text-2xl lg:text-3xl font-bold mb-12">Únete a la comunidad de músicos más grande de Chile</h2>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-3 px-6 py-4 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            <div className="relative">
              <div className="rounded-3xl p-8 shadow-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
...
function FeatureCardLarge({
  ...
  return (
    <div className="p-6 rounded-2xl transition-all hover:shadow-lg" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-4" style={{ background: `${color}15` }}>
...
function StatItem({ number, label, icon }: { number: string; label: string; icon?: React.ReactNode }) {
  return (
    <div>
      ...
      <p className="text-sm" style={{ color: 'color-mix(in oklab, var(--accent-contrast) 70%, transparent)' }}>
        {label}
      </p>
    </div>
  );
...
function MockSerenataCard({ time, location, type }: { time: string; location: string; type: string }) {
  return (
    <div className="flex items-center gap-4 p-4 rounded-xl" style={{ background: 'var(--bg-subtle)' }}>
      <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'var(--accent-subtle)' }}>
        <IconConfettiFilled size={20} style={{ color: 'var(--accent)' }} />
      </div>
