...
import type { BankTransferData } from '@simple/utils';

export type ProviderBankTransferData = BankTransferData;
...
export type ProviderGroup = {
    ...
    servicesPreview?: Pick<ProviderGroupService, 'id' | 'name' | 'price' | 'promoPrice' | 'musiciansCount' | 'durationMinutes' | 'songsIncluded' | 'isOnline' | 'isPresential'>[];
    createdAt: string;
    updatedAt: string;
};
...
export type ProviderGroupServicePack = {
    id: string;
    providerGroupId: string;
    name: string;
    description: string | null;
...
export type ProviderGroupServicePromotion = {
    id: string;
    providerGroupId: string;
    label: string;
    description: string | null;
...
export const serenatasApi = {
    ...
    marketplaceGroupServicePacks: (groupId: string) => request<{ items: ProviderGroupServicePack[] }>(`/marketplace/groups/${groupId}/service-packs`),
    marketplaceGroupServicePromotions: (groupId: string) => request<{ items: ProviderGroupServicePromotion[] }>(`/marketplace/groups/${groupId}/service-promotions`),
    marketplaceGroupCatalog: (groupId: string) => request<ProviderGroupCatalog>(`/marketplace/groups/${groupId}/catalog`),
    ...
    updateProviderGroupAvailability: (
        groupId: string,
        payload: Partial<Pick<ProviderGroupAvailability, 'slaHours' | 'bookingMode' | 'bufferMinutes' | 'alwaysOpen' | 'scheduleNote'>> & {
            rules?: ProviderGroupAvailabilityRule[];
        },
    ...
    providerGroupServicePacks: (groupId: string) => request<{ items: ProviderGroupServicePack[] }>(`/provider-groups/${groupId}/service-packs`),
    createProviderGroupServicePack: (groupId: string, payload: Partial<ProviderGroupServicePack> & { name: string; price: number; sessionsCount: number }) =>
        request<{ item: ProviderGroupServicePack }>(`/provider-groups/${groupId}/service-packs`, { method: 'POST', body: JSON.stringify(payload) }),
    updateProviderGroupServicePack: (groupId: string, packId: string, payload: Partial<ProviderGroupServicePack>) =>
        request<{ item: ProviderGroupServicePack }>(`/provider-groups/${groupId}/service-packs/${packId}`, { method: 'PATCH', body: JSON.stringify(payload) }),
    deleteProviderGroupServicePack: (groupId: string, packId: string) =>
        request<Record<string, never>>(`/provider-groups/${groupId}/service-packs/${packId}`, { method: 'DELETE' }),
    providerGroupServicePromotions: (groupId: string) => request<{ items: ProviderGroupServicePromotion[] }>(`/provider-groups/${groupId}/service-promotions`),
    createProviderGroupServicePromotion: (groupId: string, payload: Partial<ProviderGroupServicePromotion> & { label: string; discountValue: number }) =>
        request<{ item: ProviderGroupServicePromotion }>(`/provider-groups/${groupId}/service-promotions`, { method: 'POST', body: JSON.stringify(payload) }),
    updateProviderGroupServicePromotion: (groupId: string, promoId: string, payload: Partial<ProviderGroupServicePromotion>) =>
        request<{ item: ProviderGroupServicePromotion }>(`/provider-groups/${groupId}/service-promotions/${promoId}`, { method: 'PATCH', body: JSON.stringify(payload) }),
    deleteProviderGroupServicePromotion: (groupId: string, promoId: string) =>
        request<Record<string, never>>(`/provider-groups/${groupId}/service-promotions/${promoId}`, { method: 'DELETE' }),
