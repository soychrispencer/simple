...
const PersonalDataForm: React.FC<{ user: any; onSave?: (data: any) => void }> = ({ user, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      logError("[PersonalDataForm] unexpected", err);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl bg-[var(--field-bg)] border border-[var(--field-border)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="relative" ref={triggerRef}>
          ...
          {showCalendar ? (
            <div
              ref={calendarRef}
              className="absolute z-20 mt-2 w-full max-w-xs rounded-xl card-surface card-surface-raised border border-[var(--field-border)] shadow-card"
              role="dialog"
            >
              <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--field-border)]">
                <button type="button" onClick={() => setMonthOffset(-1)} className="p-1 rounded hover:bg-light-100 dark:hover:bg-dark-700">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <button
                      ...
                      className="inline-flex items-center gap-1 h-8 px-3 text-sm rounded-md border border-[var(--field-border)] bg-[var(--field-bg)] text-[var(--field-text)] hover:border-[var(--field-border-hover)] hover:bg-[var(--field-bg-hover)] focus-visible:outline-none focus-visible:ring-inset focus-visible:ring-1 focus-visible:ring-[color:var(--color-primary-a40)]"
                    >
                      ...
                      <div className="absolute left-0 mt-1 w-36 max-h-60 overflow-y-auto rounded-md card-surface card-surface-raised border border-[var(--field-border)] shadow-card py-1 z-10">
                        {monthOptions.map((month) => (
                          <button
                            key={month.value}
                            type="button"
                            className={`w-full text-left px-3 py-2 text-sm capitalize hover:bg-light-100 dark:hover:bg-dark-700 ${month.value === viewDate.getMonth() ? "font-semibold text-primary" : "text-[var(--field-text)]"}`}
                            onClick={() => {
                      ...
                      <div className="absolute left-0 mt-1 w-28 max-h-60 overflow-y-auto rounded-md card-surface card-surface-raised border border-[var(--field-border)] shadow-card py-1 z-10">
                        {yearOptions.map((year) => (
                          <button
                            key={year}
                            type="button"
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-light-100 dark:hover:bg-dark-700 ${year === viewDate.getFullYear() ? "font-semibold text-primary" : "text-[var(--field-text)]"}`}
                            onClick={() => {
                ...
                {calendarDays.map((date) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isFuture}
                      className={`h-9 text-sm rounded-md transition-colors ${isSelected ? "bg-primary text-[var(--color-on-primary)]" : ""} ${!isSelected ? "hover:bg-light-100 dark:hover:bg-dark-700" : ""} ${!isCurrentMonth ? "text-lighttext/50 dark:text-darktext/50" : "text-lighttext dark:text-darktext"} ${isFuture ? "opacity-40 cursor-not-allowed hover:bg-transparent" : ""}`}
                    >
                      {date.getDate()}
                    </button>
                  );
