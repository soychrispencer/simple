...
export default function VehicleListingCard({ data, mode }: Props) {
    ...
    const detailItems = data.meta.slice(0, 5);
    const secondaryBadgeLabel = data.live ? 'En vivo' : undefined;
    const secondaryPriceInfo =
        data.variant === 'auction' && data.auctionTime ? `Cierra en ${data.auctionTime}` : undefined;
    const temporalLabel = data.listedSince ?? (data.live ? 'En vivo ahora' : undefined);

    const engagementItems: Array<{ key: string; value: string; icon: React.ReactNode }> = [];
    if (temporalLabel) {
        engagementItems.push({ key: 'listed', value: temporalLabel, icon: <IconClock size={12} /> });
    }
            ...
            {isImageUrl ? (
                <>
                    ...
                    <Image
                        ...
                        alt={data.title}
                        fill
                        className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 motion-reduce:transition-none ${imageFailed ? 'opacity-0' : 'opacity-100'}`}
        ...
        return (
            ...
            >
                ...
                <div className="absolute top-3 right-3 z-30">
                    <PanelIconButton
                        type="button"
                        label={favorite ? 'Quitar de guardados' : 'Guardar publicación'}
                        variant="overlay"
                        size="sm"
                ...
                <div className="w-full h-40 sm:h-44 xl:h-auto xl:min-h-[180px] aspect-[3/4] sm:aspect-4/3 rounded-lg overflow-hidden shrink-0 relative transition-all duration-300 motion-reduce:transition-none" style={{ background: 'var(--bg-muted)' }}>
                    {isImageUrl ? (
                        <Image
                            ...
                            onError={() => setImageFailed(true)}
                            sizes="(max-width: 640px) 100px, (max-width: 1024px) 140px, 280px"
                            decoding="async"
                        />
                    ) : (
                        <div className="absolute inset-0 animate-pulse bg-gradient-to-br from-slate-800 to-slate-900" style={{ background: current }} />
