...
export default function GalleryImageUploader({ value = [], onChange, multiple = true, max = 12 }: Props) {
	...
	const onRemove = async (id: string) => {
		...
		if (removed?.remoteUrl) {
			...
			if (path) {
				...
				logDebug('[GalleryImageUploader] Imagen eliminada del storage', { path });
			}
		}
	...
	return (
		<div>
			<div
				className="card-surface shadow-card border border-dashed border-border/60 rounded-xl p-6 text-center text-lighttext/80 dark:text-darktext/80"
					...
					{items.map((it, idx) => (
						<div key={it.id} className="relative group">
							...
							<div className="absolute inset-0 bg-[var(--overlay-scrim-0)] group-hover:bg-[var(--overlay-scrim-40)] transition rounded-lg flex items-end p-2 gap-2 opacity-0 group-hover:opacity-100">
								<Button type="button" size="sm" className={`${it.cover ? 'bg-primary text-[var(--color-on-primary)]' : 'bg-[var(--overlay-highlight-80)]'} px-2 py-1 text-xs`} onClick={() => setCover(it.id)}>
