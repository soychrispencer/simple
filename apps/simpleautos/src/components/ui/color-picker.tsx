    ...
    return (
        <div ref={containerRef} className="relative">
            ...
            >
                {matched ? (
                    <>
                        <span className="h-3.5 w-3.5 rounded-full flex-shrink-0" style={{ background: matched.hex, border: '1px solid rgba(0,0,0,0.18)' }} />
                        <span className="flex-1 text-sm text-[var(--fg)]">{matched.label}</span>
                    </>
                ) : value ? (
                    <>
                        <span className="h-3.5 w-3.5 rounded-full flex-shrink-0 bg-[var(--bg-muted)]" style={{ border: '1px solid rgba(0,0,0,0.12)' }} />
                ...
                <div className="absolute z-50 top-full left-0 right-0 mt-1 rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg)] shadow-lg p-1.5">
                    <div className="flex flex-col max-h-60 overflow-y-auto">
                        {VEHICLE_COLORS.map(({ label, hex }) => (
                            ...
                            >
                                <span className="h-4 w-4 rounded-full flex-shrink-0" style={{ background: hex, border: '1px solid rgba(0,0,0,0.18)' }} />
