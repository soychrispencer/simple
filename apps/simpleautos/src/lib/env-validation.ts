...
export function validateEnvVars(): { valid: boolean; errors: string[] } {
    ...
    if (process.env.NODE_ENV === 'development') {
        ...
        for (const { key, value, name } of recommendedVars) {
            if (!value) {
                console.warn(`[SimpleAutos] ${name} (${key}) no está configurada. Algunas funciones pueden no estar disponibles.`);
            }
        }
    }

    if (errors.length > 0) {
        console.error('[SimpleAutos] Errores de configuración:', errors);
    }
