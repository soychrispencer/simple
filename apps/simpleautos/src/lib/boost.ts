...
export async function fetchBoostCatalog(): Promise<BoostCatalogResponse | null> {
    return apiRequest<BoostCatalogResponse>('/api/boost/catalog?vertical=autos', { method: 'GET' });
}

export async function fetchBoostOrders(): Promise<BoostOrder[]> {
    const data = await apiRequest<BoostOrdersResponse>('/api/boost/orders?vertical=autos', { method: 'GET' });
    return data?.orders ?? [];
}
...
export async function fetchFeaturedBoosted(section: BoostSection, limit = 8): Promise<FeaturedBoostItem[]> {
    const data = await apiRequest<FeaturedResponse>(
        `/api/boost/featured?vertical=autos&section=${encodeURIComponent(section)}&limit=${encodeURIComponent(String(limit))}`,
        { method: 'GET' }
    );
    return data?.items ?? [];
}
