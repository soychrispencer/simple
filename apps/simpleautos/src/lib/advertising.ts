...
export const AD_UPDATE_EVENT = 'simpleautos:ad-campaigns-updated';
...
export async function fetchMyAdCampaigns(): Promise<{
    ...
    const { status, data } = await apiRequest<CampaignListResponse>('/api/advertising/campaigns?vertical=autos', {
        method: 'GET',
    });
...
export async function fetchPublicAdCampaigns(): Promise<AdCampaign[]> {
    const { data } = await apiRequest<CampaignListResponse>('/api/advertising/public?vertical=autos', {
        method: 'GET',
    });
