...
const DESTINATIONS: Array<{ id: AdDestinationType; label: string; hint: string; badge?: string }> = [
  ...
  },
  {
    ...
    hint: 'Abre una publicación dentro de SimpleAutos.',
  },
  {
    id: 'profile',
      ...
      {activeTab === 'boost' ? (
        ...
        <>
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-6">
            <div className="autos-stat-tile"><p className="type-caption text-[var(--fg-muted)]">Vigentes</p><p className="text-xl font-semibold">{counters.totalNotEnded}/{MAX_CAMPAIGNS_TOTAL}</p></div>
            <div className="autos-stat-tile"><p className="type-caption text-[var(--fg-muted)]">Hero activas</p><p className="text-xl font-semibold">{counters.activeHero}/{MAX_ACTIVE_HERO_CAMPAIGNS}</p></div>
            <div className="autos-stat-tile"><p className="type-caption text-[var(--fg-muted)]">Programadas</p><p className="text-xl font-semibold">{counters.scheduled}</p></div>
            <div className="autos-stat-tile"><p className="type-caption text-[var(--fg-muted)]">Costo referencial</p><p className="text-xl font-semibold">${selected.price[duration].toLocaleString('es-CL')}</p></div>
          </div>
          ...
          {isEditing && editingCampaign ? (
            <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border px-3 py-2 autos-subtle-card">
        ...
        {step === 0 ? (
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {FORMATS.map((item) => (
                <PanelChoiceCard key={item.id} onClick={() => { if (!lockCommercialFields) setFormat(item.id); }} disabled={lockCommercialFields} selected={format === item.id}>
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-3 autos-icon-tile" >{item.id === 'hero' ? <IconLayoutBoard size={18} /> : item.id === 'card' ? <IconPhoto size={18} /> : <IconRectangle size={18} />}</div>
            ...
            <div className="autos-stat-tile">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <span
                          className={`mt-0.5 w-4 h-4 rounded-full border inline-flex items-center justify-center autos-radio-ring ${active ? "autos-radio-ring--active" : ""}`}
                          
                          aria-hidden
                        >
                          {active ? (
                            <span className="w-2 h-2 rounded-full autos-radio-dot" />
                          ) : null}
                        </span>
                        <span className="min-w-0">
                          <span className="text-sm font-medium block autos-text-fg">
                            {option.label}
                          </span>
                          <span className="type-caption block mt-0.5 autos-text-muted">
                            {option.hint}
                          </span>
                        </span>
                      </div>
                          {option.badge ? (
                        <span className="inline-flex mt-2 rounded-full border px-2 py-0.5 text-[11px] font-medium autos-dest-badge">
                          {option.badge}
                        </span>
                      ) : null}
                    </PanelChoiceCard>
              ...
              <div className="mt-3 rounded-lg border px-3 py-2 autos-subtle-box">
                  ...
                  {INLINE_SECTION_OPTIONS.map((sectionOption) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg border p-2 text-sm disabled:cursor-not-allowed autos-inline-pick ${active ? "autos-inline-pick--active" : ""}`} style={{ opacity: lockCommercialFields && !active ? 0.6 : 1 }}
                      >
                        {sectionOption.label}
                      </button>
                    );
        ...
        {step === 1 ? (
          <div className="space-y-5">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="autos-stat-tile"><p className="text-sm font-medium mb-2">Imagen desktop ({selected.desktop[0]}x{selected.desktop[1]})</p><input type="file" accept="image/*" onChange={(event) => void onUpload('desktop', event)} className="form-input h-auto py-2" />{desktopAsset ? <p className="type-caption mt-2 autos-text-muted">{desktopAsset.name} · {desktopAsset.width}x{desktopAsset.height}</p> : null}</div>
              <div className="autos-stat-tile"><p className="text-sm font-medium mb-2">Imagen móvil opcional ({selected.mobile[0]}x{selected.mobile[1]})</p><input type="file" accept="image/*" onChange={(event) => void onUpload('mobile', event)} className="form-input h-auto py-2" />{mobileAsset ? <p className="type-caption mt-2">{mobileAsset.name} · {mobileAsset.width}x{mobileAsset.height}</p> : null}</div>
            </div>
            ...
            <div className="rounded-xl border p-3 autos-subtle-box">
              <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
              ...
              <div className="rounded-lg border overflow-hidden autos-preview-frame" style={{ aspectRatio: `${previewSpec[0]} / ${previewSpec[1]}`, background: previewImage ? `linear-gradient(to right, rgba(0,0,0,0.45), rgba(0,0,0,0.2)), url(${previewImage}) center / cover no-repeat` : undefined }}>
        ...
        {step === 2 ? (
          <div className="space-y-4">
            <div className="autos-stat-tile p-4"><label className="type-caption font-medium mb-1 block text-[var(--fg-secondary)]">Fecha inicio</label><div className="flex items-center gap-2"><IconCalendar size={14} /><input type="date" className="form-input max-w-[220px] disabled:cursor-not-allowed" value={startDate} onChange={(event) => setStartDate(event.target.value)} disabled={lockCommercialFields} /></div></div>
            <PanelSummaryCard
              eyebrow="Resumen"
          ...
          <div className="space-y-3">
            {[...campaigns]
              ...
              .map((campaign) => {
                ...
                return (
                  <div key={campaign.id} className="rounded-xl border p-3 sm:p-4 flex flex-col gap-3 sm:flex-row sm:justify-between autos-border-card">
                    <div className="min-w-0">
                      ...
                      <p className="type-caption mt-1 autos-text-secondary">
                      ...
                      {href !== '#' ? (
                        <Link href={href} target={isExternal ? '_blank' : undefined} rel={isExternal ? 'noopener noreferrer' : undefined} className="inline-flex mt-2 text-xs font-medium autos-text-secondary">
                          Abrir destino
                        </Link>
                      ) : null}
                    </div>
