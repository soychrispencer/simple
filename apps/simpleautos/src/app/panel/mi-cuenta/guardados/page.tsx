...
import VehicleListingCard from '@/components/listings/vehicle-listing-card';
import { mapSavedListingToVehicleCard } from '@/lib/saved-listing-card';
import {
    ACCOUNT_SAVED_PAGE,
...
export default function GuardadosCuentaPage() {
    ...
    const [viewMode, setViewMode] = useState<ViewMode>(() => {
        ...
        const saved = window.localStorage.getItem('simpleautos:panel:guardados:viewMode');
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            window.localStorage.setItem('simpleautos:panel:guardados:viewMode', viewMode);
        }
    }, [viewMode]);

    const description = useMemo(() => {
        if (items.length === 0) return ACCOUNT_SAVED_PAGE.description;
        return `${items.length} vehículo${items.length === 1 ? '' : 's'} guardado${items.length === 1 ? '' : 's'}`;
    }, [items.length]);
            ...
            {items.length === 0 ? (
                ...
                <div className={viewMode === 'grid' ? 'listings-grid grid w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3' : 'w-full space-y-3'}>
                    {items.map((item) => (
                        <VehicleListingCard
                            key={item.id}
                            data={mapSavedListingToVehicleCard(item)}
                            mode={viewMode}
                        />
                    ))}
                </div>
