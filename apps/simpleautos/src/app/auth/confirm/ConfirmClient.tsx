...
export default function ConfirmClient({ emailFromQuery = '', confirmedFlag = false }: { emailFromQuery?: string, confirmedFlag?: boolean }) {
  ...
  const isEmailConfirmationFlow = !!email || authType === 'signup' || authType === 'invite' || authType === 'email_change' || authType === 'magiclink' || authType === 'recovery';
    ...
    (async () => {
      ...
      try {
        ...
        if (code) {
          ...
          if (error) {
            if (active) setConfirmError('No pudimos validar tu acceso. Intenta nuevamente o solicita un nuevo correo.');
            return;
          }
  ...
  useEffect(() => {
    ...
    const msg = authType === 'email_change'
  ...
  // Para OAuth (Google) queremos redirección inmediata sin "flash" visual.
  // Si algo falla, mostramos una pantalla mínima con el error.
  if (!isEmailConfirmationFlow) {
    if (confirmError) {
      return (
