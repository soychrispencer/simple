...
const PillButton: React.FC<{ active: boolean; onClick: () => void; label: string }> = ({ active, onClick, label }) => (
  <button
    type="button"
    onClick={onClick}
    className={`px-3 h-8 rounded-full text-xs font-medium border transition focus:outline-none focus-visible:ring-1 focus-visible:ring-black dark:focus-visible:ring-white inline-flex items-center
      ${active
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {ALL_COLORS.length > limitCollapsed && (
          <button
            ...
            className="px-3 h-8 rounded-full text-xs font-medium border border-dashed border-[var(--field-border)] text-gray-600 dark:text-gray-300 hover:bg-[var(--field-bg-hover)] focus:outline-none focus-visible:ring-1 focus-visible:ring-black dark:focus-visible:ring-white"
          >{expanded ? 'Menos' : 'Más'}</button>
        )}
      </div>
      {value.length > 0 && (
        <p className="mt-2 text-[11px] text-gray-500 dark:text-gray-400">{value.length} seleccionado{value.length>1?'s':''}</p>
      )}
    </div>
  );
};
