...
// ============ CAR (vehicle_cars) ============
// Orden solicitado para autos:
// Carrocería, Versión, Transmisión, Tracción, Combustible, Motor, Potencia, Consumo, Puertas, Nº de dueño, Garantía
// (Se mantiene misma configuración de opciones y validaciones)
const carFields: SpecFieldDescriptor[] = [
  ...
  { id: 'version', label: 'Versión', type: 'text', required: false, placeholder: 'Ej: GLi Sport' },
  { id: 'transmission', label: 'Transmisión', type: 'select', required: false, options: [
    { value: 'manual', label: 'Manual' },
    { value: 'automatic', label: 'Automática' },
    { value: 'cvt', label: 'CVT' },
  ] },
  { id: 'drivetrain', label: 'Tracción', type: 'select', required: false, options: [
    { value: 'fwd', label: 'FWD' },
    { value: 'rwd', label: 'RWD' },
    { value: 'awd', label: 'AWD' },
    { value: '4x4', label: '4x4' },
  ...
  { id: 'engine', label: 'Motor', type: 'text', required: false, placeholder: 'Ej: 2.0L' },
  { id: 'power', label: 'Potencia', type: 'text', required: false, placeholder: 'Ej: 150hp' },
  { id: 'fuel_consumption', label: 'Consumo', type: 'text', required: false, placeholder: 'Ej: 14 km/l' },
  { id: 'doors', label: 'Puertas', type: 'number', required: false, min: 2, max: 7 },
  { id: 'owners_count', label: 'Nº dueños', type: 'number', required: false, min: 0, max: 15 },
  { id: 'warranty', label: 'Garantía', type: 'text', required: false, placeholder: 'Ej: 1 año o 10.000 km' },
];
...
const motorcycleFields: SpecFieldDescriptor[] = [
  { id: 'displacement_cc', label: 'Cilindrada (cc)', type: 'number', unit: 'cc', required: false, min: 50, max: 2500 },
...
// ============ COMMERCIAL (vehicle_commercial) ============
const commercialFields: SpecFieldDescriptor[] = [
  { id: 'cargo_volume_m3', label: 'Volumen carga (m³)', type: 'number', unit: 'm³', required: false, min: 0, max: 200 },
  { id: 'payload_capacity_kg', label: 'Carga útil (kg)', type: 'number', unit: 'kg', required: false, min: 0, max: 10000 },
  { id: 'wheelbase_mm', label: 'Distancia ejes (mm)', type: 'number', unit: 'mm', required: false, min: 500, max: 7000 },
  { id: 'fuel_type', label: 'Combustible', type: 'select', required: false, options: [
    { value: 'gasoline', label: 'Gasolina' },
    { value: 'diesel', label: 'Diésel' },
    { value: 'hybrid', label: 'Híbrido' },
  ...
  { id: 'doors', label: 'Puertas', type: 'number', required: false, min: 2, max: 6 },
];

// ============ INDUSTRIAL (vehicle_industrial) ============
const industrialFields: SpecFieldDescriptor[] = [
...
// (Eliminado soporte 'van' a petición: cualquier alias furgon/furgón se redirige a 'commercial')
...
export const specCategories: Record<string, SpecCategoryDescriptor> = {
  ...
  commercial: { type_key: 'commercial', fields: commercialFields, schema: buildSchema(commercialFields) },
  industrial: { type_key: 'industrial', fields: industrialFields, schema: buildSchema(industrialFields) },
};
...
const aliasMap: Record<string,string> = {
  ...
  suv: 'car',
  vans: 'commercial', furgon: 'commercial', furgón: 'commercial', furgoneta: 'commercial'
};
...
export function validateSpecsWithRules(typeKeyRaw: string | null | undefined, specs: Record<string, any>): CrossFieldValidationResult {
  ...
  for (const k of Object.keys(s)) {
    if (s[k] === '') {
      // Cadena vacía cuenta como ausencia -> dejar que capa superior la filtre si quiere.
      continue;
    }
  }
  ...
  switch (typeKey) {
    ...
    case 'commercial': {
      const vol = num(s.cargo_volume_m3);
      if (vol && vol > 500) {
        errors.cargo_volume_m3 = 'Volumen de carga inusualmente alto (>500 m³)';
      }
      break;
    }
    case 'car': {
