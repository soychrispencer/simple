...
import { deleteVehicleImage, extractPathFromUrl } from "@/lib/supabaseStorage";
...
export default function GalleryImageUploader({ value = [], onChange, multiple = true, max = 12 }: Props) {
	...
	return (
		<div>
			<div
				className="border-2 border-dashed rounded-xl p-6 text-center text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-900"
				...
				<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
					{items.map((it, idx) => (
						<div key={it.id} className="relative group">
							...
							<div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition rounded-lg flex items-end p-2 gap-2 opacity-0 group-hover:opacity-100">
								<Button type="button" size="sm" className={`${it.cover ? 'bg-green-600 text-white' : 'bg-white/80'} px-2 py-1 text-xs`} onClick={() => setCover(it.id)}>
