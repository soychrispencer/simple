...
type FeaturedGroups = Record<ListingGroup, Vehicle[]>;
...
const emptyGroups: FeaturedGroups = {
  sale: [],
  rent: [],
  auction: [],
};

function groupFeatured(items: Vehicle[]): FeaturedGroups {
  return items.reduce<FeaturedGroups>((acc, vehicle) => {
    const key = (vehicle.listing_type || vehicle.listing_kind || "sale") as ListingGroup;
    if (acc[key]) {
      acc[key] = acc[key].concat(vehicle);
    }
    return acc;
  }, { ...emptyGroups });
}
...
export const FeaturedVehiclesSlider: React.FC<FeaturedVehiclesSliderProps> = ({ slotKey, userId, limit = 45 }) => {
    ...
    (async () => {
      try {
        ...
        if (slotKey) {
          ...
          if (data.length === 0) {
            console.log(`No vehicles in slot ${slotKey}`);
          }
        } else {
          // Método tradicional
        ...
        if (!cancelled) {
          setItems(data.map(row => {
            ...
            const seller = (row as any).profiles ? {
              ...
              email: (row as any).profiles.email || undefined,
              phone: (row as any).profiles.phone || undefined,
            } : undefined;
  ...
  if (error) {
    return (
      <section className="w-full max-w-full mx-auto mt-6 px-4 md:px-8 text-center text-sm text-red-500">
        Error cargando vehículos impulsados: {error}
      </section>
    );
  }

  // No mostrar nada durante la carga ni mensaje si no hay vehículos (evita intermitencia visual)
