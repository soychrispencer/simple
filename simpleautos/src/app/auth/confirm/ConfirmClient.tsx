...
export default function ConfirmClient({ emailFromQuery = '', confirmedFlag = false }: { emailFromQuery?: string, confirmedFlag?: boolean }) {
  ...
  async function handleResend() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        addToast('Correo de confirmación reenviado.', { type: 'success' });
      }
    } catch {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-xl w-full bg-white dark:bg-gray-800 shadow-md rounded-lg p-8">
        {isConfirmed ? (
          <div className="text-center">
            <h1 className="text-2xl font-semibold text-black dark:text-white mb-3">¡Correo confirmado exitosamente!</h1>
            <p className="text-gray-600 dark:text-gray-300 mb-6">Tu cuenta ya está lista. Serás redirigido al panel en <strong>{count}</strong> segundos...</p>
            <Button onClick={() => router.push('/panel')} variant="primary">Ir al Panel</Button>
            <div className="mt-4">
              <button type="button" className="text-primary underline" onClick={() => openAuthModal('login')}>¿Ya tienes cuenta? Inicia sesión</button>
            </div>
          </div>
        ) : (
          <div className="text-center">
            <h1 className="text-2xl font-semibold text-black dark:text-white mb-3">Confirma tu correo</h1>
            <p className="text-gray-600 dark:text-gray-300 mb-4">Hemos enviado un correo de confirmación a: <strong>{email || 'tu correo'}</strong></p>
            ...
            <div className="text-xs text-gray-600 mt-4">¿No llegó? Revisa la carpeta de spam, espera unos minutos o prueba reenviar.</div>
          </div>
        )}
      </div>
    </div>
