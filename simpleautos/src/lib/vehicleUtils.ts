...
export function mapVehicleToFeaturedRow(r: VehicleJoinedRow, profilesMap: Record<string, Profile>): FeaturedVehicleRow {
  ...
  return {
    ...
    type_key: r.vehicle_types?.slug ?? null,
    type_label: r.vehicle_types?.label ?? null,
...
/**
 * Función común para cargar perfiles de propietarios
 */
...
/**
 * Función común para mapear VehicleRow a Vehicle en páginas de categorías
 */
export function mapVehicleRowToVehicle(row: VehicleRowForMapping): Vehicle {
  ...
  return {
    ...
    owner_id: '',
    type_id: row.type_id || '',
    type_key: (row.vehicle_types?.slug as VehicleCategory) || null,
    title: row.title,
    description: null,
