...
export function legacyToCanon({ basic, vehicle, commercial }: LegacyWizardSlice = {}): VehiculoWizard {
  ...
  const estado = (basic?.estado ?? basic?.condition ?? basic?.state ?? vehicle?.condition ?? vehicle?.state ?? null) as EstadoVehiculo | null;
  ...
  const historial = Array.from(new Set(historialBase?.filter(Boolean))) as HistorialVehiculoTag[];
...
export type VehicleCategory = 'car' | 'motorcycle' | 'truck' | 'bus' | 'industrial' | 'commercial';
